...
- 禁止操作：安装依赖；真实或付费 API；正式画布写入；正式软件启停；Git 推送、合并、变基、破坏性回退或无关文件暂存；清理用户工作树。验证通过后的任务范围内本地恢复提交按现行契约允许执行。
- 当前任务：智能画布引用修复队列 R1–R4 已完成；正在收敛最终验收与恢复记录，不再扩大生产代码范围。
...
- 当前状态：Batch 14 与 U1–U6 仍保持稳定；R1–R4 已分批建立恢复点。资产图、分镜/逐镜图和视频引用身份、顺序、角色、完整性锁及重启恢复均已由假 Provider 证明；RunningHub、MiniMax 与三类图片入口的稳定红灯也已修复。没有真实 API、付费调用或正式画布写入。
