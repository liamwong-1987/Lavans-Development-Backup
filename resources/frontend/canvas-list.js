...
async function loadAll(){
    try {
        ...
        resetView();
        refreshTrashCount();
        // 如果有 lastCanvasId 记忆且 URL 没强制显示列表（?view=list），自动直入画布（侧栏"无限画布"点回来时返回原画布）。
        const lastId = rememberedLastCanvasId();
        const forceListView = new URLSearchParams(window.location.search).get('view') === 'list';
        if(lastId && !forceListView){
            const lastCanvas = canvases.find(c => c.id === lastId && !c.deleted_at);
            if(lastCanvas){
                if(lastCanvas.project && lastCanvas.project !== currentProjectId){
                    rememberProjectId(lastCanvas.project);
                    currentProjectId = lastCanvas.project;
                    renderProjects();
                }
...
async function renameProject(pid, name){
    ...
    try {
        const res = await fetch(`/api/canvas/projects/${encodeURIComponent(pid)}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name })
        });
...
async function deleteProject(pid){
    ...
    try {
        const res = await fetch(`/api/canvas/projects/${encodeURIComponent(pid)}`, { method: 'DELETE' });
...
function autoLayoutNulls(items){
    // grid layout for cards with null position; also repair legacy overlapping cards once.
    const X0 = 40, Y0 = 40, XSTRIDE = 276, YSTRIDE = 176, COLS = 4;
    const CARD_W = 252, CARD_H = 152;
    const occupied = [];
    const overlaps = candidate => occupied.some(item =>
        candidate.x < item.x + CARD_W && candidate.x + CARD_W > item.x &&
        candidate.y < item.y + CARD_H && candidate.y + CARD_H > item.y
    );
    const reserve = (canvas, position) => {
        canvas.board_x = position.x;
        canvas.board_y = position.y;
        occupied.push(position);
        persistMeta(canvas.id, { board_x: position.x, board_y: position.y });
    };
    let slot = 0;
    items.forEach(canvas => {
        const current = { x: Math.round(Number(canvas.board_x) || 0), y: Math.round(Number(canvas.board_y) || 0) };
        if(canvas.board_x == null || canvas.board_y == null || overlaps(current)){
            while(slot < 256){
                const col = slot % COLS;
                const rowIdx = Math.floor(slot / COLS);
                const candidate = { x: X0 + col * XSTRIDE, y: Y0 + rowIdx * YSTRIDE };
                slot += 1;
                if(!overlaps(candidate)){
                    reserve(canvas, candidate);
                    break;
                }
            }
        } else {
...
function openCanvas(c){
    ...
    window.location.href = (c.kind === 'smart')
        ? `/smart-canvas-core/smart-canvas.html?id=${enc}&project=${project}&system=canvas&v=2026.08.13.1`
        : `/canvas.html?id=${enc}&project=${project}&system=canvas&v=2026.08.13.1`;
}
...
function nextCanvasBoardPosition(worldPt){
    const origin = {
        x: Math.round(Number(worldPt?.x) || 40),
        y: Math.round(Number(worldPt?.y) || 40)
    };
    const occupied = canvasesInProject(currentProjectId).map(c => ({
        x: Math.round(Number(c.board_x) || 0),
        y: Math.round(Number(c.board_y) || 0)
    }));
    const strideX = 276;
    const strideY = 176;
    const cardWidth = 252;
    ...
    const overlaps = candidate => occupied.some(item =>
        candidate.x < item.x + cardWidth && candidate.x + cardWidth > item.x &&
        candidate.y < item.y + cardHeight && candidate.y + cardHeight > item.y
    );
    for(let index = 0; index < 64; index += 1){
        const col = index % 4;
        const row = Math.floor(index / 4);
        const candidate = { x: origin.x + col * strideX, y: origin.y + row * strideY };
        if(!overlaps(candidate)) return candidate;
    }
    return { x: origin.x + strideX, y: origin.y + strideY };
}

async function createCanvasOnBoard(title, kind, worldPt){
    const isSmart = kind === 'smart';
    const boardPosition = nextCanvasBoardPosition(worldPt);
    ...
    try {
        ...
        if(nc){
            ...
            if(nc.board_x == null) nc.board_x = boardPosition.x;
            if(nc.board_y == null) nc.board_y = boardPosition.y;
            canvases.push(nc);
            renderBoard();
            renderProjects();
        }
...
async function exportCanvas(id){
    ...
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}`);
...
async function deleteCanvas(id){
    ...
    if(!c) return;
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}`, { method: 'DELETE' });
...
async function restoreCanvas(id){
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}/restore`, { method: 'POST' });
...
async function purgeCanvas(id){
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}/purge`, { method: 'DELETE' });
