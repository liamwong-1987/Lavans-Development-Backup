(function(){
    ...
    function isScaleHost(){
        return !isFramed() && !!document.querySelector('.app-shell iframe, iframe.active');
    }
    ...
    function updateScaleBodyClasses(){
        ...
        document.body.classList.toggle('studio-scale-host', isScaleHost());
    ...
    function broadcastScale(mode){
        const frameMode = isScaleHost() ? '100' : mode;
        const frameScale = isScaleHost() ? 1 : appliedScale();
        document.querySelectorAll('iframe').forEach(frame => {
            try {
                frame.contentWindow?.postMessage({ type: 'studio-ui-scale', mode: frameMode, scale: frameScale }, '*');
            } catch(e) {}
        });
    }
