// comfyui-settings.js — 工作流设置页（像素级复刻大神，适配 Lavans API）
const I18N = {
    'comfy.title': '工作流设置',
    ...
    'canvas.comfyCustom': 'Comfy 自定义',
    'common.delete': '删除',
    'canvas.decrease': '缩小',
    'canvas.increase': '放大'
};
function tr(key){ return (window.StudioI18n && StudioI18n.t(key)) || I18N[key] || key; }
function tf(key, vars={}){ return Object.entries(vars).reduce((text, [k,v]) => text.replaceAll(`{${k}}`, v), tr(key)); }
...
const TYPES = [
    ...
    { v:'boolean', zh:'开关', en:'Toggle' },
];
function typeLabel(type){ const item = TYPES.find(t => t.v === type); return item ? ((window.StudioI18n && StudioI18n.lang && StudioI18n.lang() === 'en') ? item.en : item.zh) : type; }
...
const INPUT_LABELS = {
    'text': '提示词文本', 'prompt': '提示词', 'positive': '正向条件', 'negative': '负向条件',
    'seed': '随机种子', 'noise_seed': '噪声种子', 'steps': '采样步数', 'cfg': 'CFG 引导系数',
    'sampler_name': '采样方法', 'scheduler': '调度器', 'denoise': '重绘强度', 'width': '宽度', 'height': '高度',
    'batch_size': '批量大小', 'megapixels': '百万像素', 'strength_model': '模型强度', 'strength_clip': 'CLIP 强度',
    'lora_name': 'LoRA 模型', 'ckpt_name': '主模型', 'vae_name': 'VAE 模型', 'clip_name': 'CLIP 模型',
    'clip_name1': 'CLIP 模型 1', 'clip_name2': 'CLIP 模型 2', 'unet_name': 'UNet 模型', 'control_net_name': 'ControlNet 模型',
    'image': '图片', 'images': '图片', 'mask': '蒙版', 'latent': '潜空间', 'value': '数值', 'string': '字符串',
    'strength': '强度', 'guidance': '引导系数', 'resolution': '分辨率', 'filename_prefix': '文件名前缀',
    'upscale_method': '放大方式', 'crop': '裁剪方式',
};

function nodeLabel(node){ if(node._meta?.title) return node._meta.title; return NODE_INFO[node.class_type]?.label || node.class_type || '未命名'; }
function nodeSub(node){ const info = NODE_INFO[node.class_type]; if(info && node._meta?.title) return info.label + ' · ' + node.class_type; return node.class_type || ''; }
function nodeIcon(node){ return NODE_INFO[node.class_type]?.icon || '◆'; }
function inputLabel(name){ return INPUT_LABELS[name] || name; }
...
function mediaFieldLabel(kind, count){ const labels = {image:'图片', video:'视频', audio:'音频'}; return `${labels[kind] || kind} ${count}`; }
function mediaAccept(kind){ if(kind === 'video') return 'video/*'; if(kind === 'audio') return 'audio/*'; return 'image/*'; }
function mediaUploadText(kind){ if(kind === 'video') return tr('comfy.clickUploadVideo'); if(kind === 'audio') return tr('comfy.clickUploadAudio'); return tr('comfy.clickUploadImage'); }
function mediaUploadFailedText(kind){ if(kind === 'video') return tr('comfy.videoUploadFailed'); if(kind === 'audio') return tr('comfy.audioUploadFailed'); return tr('comfy.imageUploadFailed'); }
function mediaPreviewHtml(kind, url, name='', compact=false){
    const safeUrl = escapeAttr(url || ''); const safeName = escapeHtml(name || typeLabel(kind));
...
function defaultMiniCards(){ return { prompt:{ x:24, y:30 }, image:{ x:24, y:210 }, custom:{ x:280, y:78 }, output:{ x:540, y:120 } }; }
...
async function saveComfyInstances(){
    ...
    setStatus('保存中...');
    try {
        const res = await fetch('/api/comfyui/instances', { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ instances: cleaned }) });
        if(!res.ok) throw new Error((await res.json()).detail || '保存失败');
        const data = await res.json();
        comfyInstances = data.instances || cleaned;
        renderComfyInstances();
        setStatus('ComfyUI 后端地址已保存');
    } catch(e){ alert(e.message || '保存失败'); setStatus('保存失败'); }
}
...
async function loadList(){
    try {
        ...
        if(!stillExists && workflows.length){ await selectWorkflow(workflows[0].name); }
    } catch(e){ setStatus(tr('comfy.loadFailed')); console.error(e); }
}
window.addEventListener('pageshow', () => { if(!currentWorkflow && workflows.length === 0) loadList(); });
...
async function selectWorkflow(name){
    ...
    try {
        ...
        previewImageUrls = {};
        previewValues = {};
        currentConfig.fields.forEach(f => { if(f.default !== undefined && f.default !== null) previewValues[f.id] = f.default; });
        previewRandomActive = {};
        runResult = null;
...
function fieldFor(node, input){ return currentConfig.fields.find(f => f.node === node && f.input === input); }
...
function toggleField(node, input){
    ...
    if(existing){
        ...
        const f = { id: makeFieldId(), node, input, name: inputLabel(input), type, default: typeof rawValue === 'object' ? null : rawValue, options: [] };
        if(type === 'slider' || type === 'number') {
            if(typeof rawValue === 'number'){ f.min = 0; f.max = Math.max(rawValue * 2, 10); f.step = rawValue > 0 && rawValue < 5 ? 0.1 : 1; }
...
function refreshPopupBody(){
    ...
    if(!body) return;
    const inputs = Object.entries(node.inputs || {}).filter(([k,v]) => !(Array.isArray(v) && v.length === 2 && typeof v[0] === 'string' && typeof v[1] === 'number'));
    body.innerHTML = inputs.length === 0 ? `<div class="popup-empty">${tr('comfy.noConfigFields')}</div>` : inputs.map(([key, value]) => renderInputRow(popupNodeId, key, value)).join('');
    refreshIcons();
}
...
function guessType(value, inputName){
    ...
    if(typeof value === 'number'){ if(/strength|cfg|denoise/.test(lc)) return 'slider'; return 'number'; }
...
function updateField(fieldId, key, value){
    ...
    if(!f) return;
    f[key] = value;
    if(key === 'type'){ previewValues[fieldId] = (value === 'boolean') ? false : (value === 'number' || value === 'slider' ? 0 : ''); f.random_enabled = value === 'number' ? !!f.random_enabled : false; delete previewRandomActive[fieldId]; }
    if(key === 'random_enabled'){ delete previewRandomActive[fieldId]; }
...
function computeLayers(){
    ...
    ids.forEach(id => {
        ...
        Object.values(inputs).forEach(v => {
            if(Array.isArray(v) && v.length === 2 && typeof v[0] === 'string'){
                if(currentWorkflow[v[0]]){ incoming[id].add(v[0]); outgoing[v[0]].add(id); }
            }
        });
    });
    const layer = {}; const visited = new Set();
    function dfs(id, lv){
        if(visited.has(id)) return;
        if((layer[id] || 0) < lv) layer[id] = lv; else layer[id] = layer[id] || lv;
        visited.add(id);
    ...
    const buckets = {};
    ids.forEach(id => { const lv = layer[id]; (buckets[lv] = buckets[lv] || []).push(id); });
    return { layer, buckets, incoming };
}

function renderGraph(){
    const svg = document.getElementById('graphSvg');
    if(!currentWorkflow || !Object.keys(currentWorkflow).length){ document.getElementById('graphCard').style.display = 'none'; return; }
    document.getElementById('graphCard').style.display = 'block';
    const { buckets } = computeLayers();
    ...
    sortedLevels.forEach(lv => {
        ...
        ids.forEach((id, idx) => { positions[id] = { x: lv * (NODE_W + X_GAP) + 16, y: idx * (NODE_H + Y_GAP) + 16 }; });
    ...
    Object.keys(currentWorkflow).forEach(toId => {
        ...
        Object.values(inputs).forEach(v => {
            if(Array.isArray(v) && v.length === 2 && typeof v[0] === 'string' && positions[v[0]]){
                ...
                seen.add(v[0]);
                const from = positions[v[0]]; const to = positions[toId];
    ...
    const nodesHtml = Object.entries(currentWorkflow).map(([id, node]) => {
        ...
        const label = nodeLabel(node); const sub = node.class_type || '';
        const exposedCount = currentConfig.fields.filter(f => f.node === id).length;
        const exposedClass = exposedCount > 0 ? 'has-exposed' : '';
        const cat = NODE_INFO[node.class_type]?.cat || 'misc'; const icon = nodeIcon(node);
...
function updateZoomPill(){ const pill = document.getElementById('zoomPill'); if(pill) pill.textContent = Math.round(graphView.k * 100) + '%'; }
function applyGraphTransform(){ const vp = document.getElementById('graphViewport'); if(vp) vp.setAttribute('transform', `translate(${graphView.x},${graphView.y}) scale(${graphView.k})`); updateZoomPill(); }
...
function attachPanZoom(svg, wrap){
    ...
    window.addEventListener('mouseup', () => { if(panState){ panState = null; wrap.classList.remove('is-panning'); } });
}
...
function openNodePopup(nodeId, gEl){
    ...
    const inputs = Object.entries(node.inputs || {}).filter(([k,v]) => !(Array.isArray(v) && v.length === 2 && typeof v[0] === 'string' && typeof v[1] === 'number'));
    const icon = nodeIcon(node); const label = nodeLabel(node); const sub = nodeSub(node);
    popup.innerHTML = `
        <div class="popup-head">
            <span class="popup-icon">${icon}</span>
            <div style="min-width:0;flex:1"><div class="popup-title">${escapeHtml(label)}</div><div class="popup-sub">${escapeHtml(sub)} · #${escapeHtml(nodeId)}</div></div>
            <div class="popup-close" onclick="closeNodePopup()"><i data-lucide="x" class="w-4 h-4"></i></div>
        </div>
        <div class="popup-body">${inputs.length === 0 ? `<div class="popup-empty">${tr('comfy.noConfigFields')}</div>` : inputs.map(([key, value]) => renderInputRow(nodeId, key, value)).join('')}</div>
    `;
    const wrap = document.querySelector('.graph-svg-wrap');
    ...
    if(left + POP_W > wrap.scrollLeft + wrap.clientWidth - 8){ left = gRect.left - wrapRect.left + wrap.scrollLeft - POP_W - 12; }
...
function renderInputRow(nodeId, inputKey, rawValue){
    ...
    if(typeOf === 'string'){ const preview = rawValue.length > 50 ? rawValue.slice(0,50) + '…' : rawValue; valueBadge = `<span style="color:var(--muted);font-size:10.5px;font-weight:700">"</span><span style="color:var(--text);font-size:11px;font-weight:700">${escapeHtml(preview)}</span><span style="color:var(--muted);font-size:10.5px;font-weight:700">"</span>`; }
    else if(typeOf === 'number'){ valueBadge = `<span style="color:#0369a1;font-size:11px;font-weight:800;font-variant-numeric:tabular-nums">${rawValue}</span>`; }
    else if(typeOf === 'boolean'){ valueBadge = `<span style="color:${rawValue?'#15803d':'#b45309'};font-size:11px;font-weight:800">${rawValue?'✓ true':'✗ false'}</span>`; }
    else { valueBadge = `<span style="color:var(--faint);font-size:11px">${escapeHtml(String(rawValue))}</span>`; }
    ...
    return `
        <div class="input-row ${active?'is-active':''} ${showExtras?'has-extras':''}">
            <div class="check-toggle ${active?'checked':''}" onclick="toggleField('${escapeAttr(nodeId)}','${escapeAttr(inputKey)}')">${active ? '<i data-lucide="check" class="w-3 h-3"></i>' : ''}</div>
...
function renderExtras(f){
    if(f.type === 'slider' || f.type === 'number'){
        const randomToggle = f.type === 'number' ? `<label class="random-toggle" onclick="event.stopPropagation()"><input type="checkbox" ${f.random_enabled === true ? 'checked' : ''} onchange="updateField('${f.id}','random_enabled',this.checked)">随机数</label>` : '';
    ...
    if(f.type === 'dropdown'){
        const opts = f.options || []; const fid = escapeAttr(f.id);
        const rows = opts.map((o, i) => {
            const looksNumber = String(o).trim() !== '' && !isNaN(Number(o));
            const tag = looksNumber ? '<span class="opt-type-tag is-num">数字</span>' : '<span class="opt-type-tag">文本</span>';
            return `<div class="dropdown-opt-row">
                <span class="opt-index">${i + 1}</span>
                <input class="small-input" type="text" placeholder="选项 ${i + 1}" value="${escapeAttr(o)}" onmousedown="event.stopPropagation()" onclick="event.stopPropagation()" oninput="updateDropdownOption('${fid}', ${i}, this.value, this)">
                ${tag}
        ...
        return `<div class="extras-row" style="flex-direction:column;align-items:stretch;gap:6px">
            <div style="font-size:11px;color:var(--muted);font-weight:700">下拉选项 <span style="color:var(--faint)">· 数字形式自动作为数值传给 ComfyUI</span></div>
            ${rows}
...
function updateDropdownOption(fieldId, index, value, inputEl){
    ...
    f.options = f.options || []; f.options[index] = value;
    if(inputEl){ const tag = inputEl.parentElement?.querySelector('.opt-type-tag'); if(tag){ const looksNumber = String(value).trim() !== '' && !isNaN(Number(value)); tag.classList.toggle('is-num', looksNumber); tag.textContent = looksNumber ? '数字' : '文本'; } }
    renderPreview();
}
function addDropdownOption(fieldId){ const f = currentConfig.fields.find(x => x.id === fieldId); if(!f) return; f.options = [...(f.options || []), '']; renderPreview(); if(popupNodeId === f.node) refreshPopupBody(); }
function removeDropdownOption(fieldId, index){ const f = currentConfig.fields.find(x => x.id === fieldId); if(!f) return; f.options = (f.options || []).filter((_, i) => i !== index); renderPreview(); if(popupNodeId === f.node) refreshPopupBody(); }
...
function randomValueForField(f){
    ...
    if(isFloat){ const precision = Math.min(8, Math.max(1, String(f.step).split('.')[1]?.length || 2)); value = Number(value.toFixed(precision)); }
    else { value = Math.floor(value); }
    return value;
}
function fieldSupportsRandom(f){ return !!f && f.type === 'number' && f.random_enabled === true; }
function isPreviewRandomActive(fieldId){ return previewRandomActive[fieldId] !== false; }
...
function applyActiveRandomValues(fields){
    ...
    currentConfig?.fields?.forEach(f => {
        if(fieldSupportsRandom(f) && isPreviewRandomActive(f.id)){ const value = randomValueForField(f); out[f.id] = value; previewValues[f.id] = value; }
    });
    return out;
}
function openImagePreview(url){ const box = document.getElementById('imageLightbox'); const img = document.getElementById('imageLightboxImg'); if(!box || !img || !url) return; img.src = url; box.classList.add('open'); }
function closeImagePreview(){ const box = document.getElementById('imageLightbox'); const img = document.getElementById('imageLightboxImg'); if(box) box.classList.remove('open'); if(img) img.src = ''; }

function renderPreview(){
    const fields = currentConfig?.fields || [];
    if(!fields.length){ previewCard.innerHTML = `<div class="preview-empty">${tr('comfy.previewEmpty')}</div>`; return; }
    const fieldsHtml = fields.map(f => renderPreviewField(f)).join('');
    const resultHtml = runResult ? `<div class="run-result"><img src="${escapeAttr(runResult)}" onclick="openImagePreview('${escapeAttr(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>` : '';
    const runButton = `<button id="runBtn" class="run-btn" type="button" onclick="onRun()"><i data-lucide="play" class="w-4 h-4"></i><span>${tr('comfy.runTest')}</span></button>`;
    previewCard.innerHTML = `${fieldsHtml}${runButton}${resultHtml}`;
    refreshIcons();
}
function renderPreviewField(f){
    ...
    if(f.type === 'textarea') return `<div class="pfield">${label}<textarea class="pfield-textarea" oninput="setPreviewValue('${f.id}',this.value)">${escapeHtml(v)}</textarea></div>`;
    if(f.type === 'number'){ const randomBtn = randomButtonHtml(f); return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="pfield-input" type="number" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value)||0)">${randomBtn}</div></div>`; }
    if(f.type === 'slider'){ const min = f.min ?? 0, max = f.max ?? 10, step = f.step ?? 1; return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider"><input type="range" min="${min}" max="${max}" step="${step}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value))"><span class="pfield-slider-val" data-slider-val="${f.id}">${v}</span></div></div></div>`; }
    if(f.type === 'dropdown'){ const opts = (f.options || []).map(o => `<option value="${escapeAttr(o)}" ${String(v)===String(o)?'selected':''}>${escapeHtml(o)}</option>`).join(''); return `<div class="pfield">${label}<select class="pfield-select" onchange="setPreviewValue('${f.id}',this.value)">${opts || `<option value="">${tr('comfy.noOptions')}</option>`}</select></div>`; }
    if(isMediaField(f)){ const displayUrl = previewImageUrls[f.id] || (typeof v === 'string' && /^(\/|https?:|blob:|data:)/.test(v) ? v : ''); return `<div class="pfield">${label}<div class="pfield-image-drop ${displayUrl?'has-image':''}" onclick="pickImage('${f.id}')">${mediaPreviewHtml(fieldKind(f), displayUrl, v)}</div></div>`; }
    if(f.type === 'boolean') return `<div class="pfield">${label}<div class="pfield-bool"><div class="pfield-bool-track ${v?'on':''}" onclick="setPreviewValue('${f.id}',!${!!v});this.classList.toggle('on')"><div class="pfield-bool-thumb"></div></div></div></div>`;
...
function renderWorkspaceView(){
    ...
    document.getElementById('workspaceGraphTab')?.classList.toggle('active', workspaceMode === 'graph');
    document.getElementById('workspaceCanvasTab')?.classList.toggle('active', workspaceMode === 'canvas');
    if(!currentWorkflow){ if(graphWrap) graphWrap.style.display = 'none'; if(miniCanvasHost) miniCanvasHost.style.display = 'none'; return; }
    if(workspaceMode === 'canvas'){ if(graphWrap) graphWrap.style.display = 'none'; if(nodesToggle) nodesToggle.style.display = 'none'; renderMiniCanvasPreview(miniCanvasHost, true); }
    else { if(graphWrap) graphWrap.style.display = 'block'; if(miniCanvasHost) miniCanvasHost.style.display = 'none'; if(nodesToggle) nodesToggle.style.display = 'flex'; }
}
...
function renderMiniCanvasPreview(target = previewCard, large = false){
    ...
    const resultHtml = runResult ? `<div class="mini-result"><img src="${escapeAttr(runResult)}" onclick="openImagePreview('${escapeAttr(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>` : `<div class="preview-empty" style="padding:18px 10px">${tr('comfy.resultHere')}</div>`;
            ...
            <div id="miniWorld" class="mini-world" style="transform:translate(${miniView.x}px,${miniView.y}px) scale(${miniView.k})">
                ...
                <div class="mini-card comfy-card" data-node="${comfy.id}" style="left:${comfy.x}px;top:${comfy.y}px">
                    ...
                    <div class="mini-card-body">
                        ...
                        <div class="preview-empty" style="padding:10px;font-size:11px;text-align:left">${mediaFieldLabel('image', imageFields.length)} · ${mediaFieldLabel('video', videoFields.length)} · ${mediaFieldLabel('audio', audioFields.length)} · ${promptFields.length ? tr('comfy.acceptsPrompt') : tr('comfy.noPromptField')}</div>
                        <div class="mini-settings-list">${settingFields.length ? settingFields.map(f => renderMiniField(f)).join('') : `<div class="preview-empty" style="padding:16px 10px">${tr('comfy.otherParamsHere')}</div>`}</div>
                        <button id="runBtn" class="run-btn mini-run" type="button" onclick="onRun()"><i data-lucide="play" class="w-4 h-4"></i><span>${tr('comfy.runTest')}</span></button>
                    </div>
                </div>
...
function renderMiniField(f){
    const label = `<div class="pfield-label">${escapeHtml(f.name || f.input)}</div>`;
    const v = previewValues[f.id] ?? f.default ?? (f.type==='boolean'?false:(f.type==='number'||f.type==='slider'?0:''));
    if(isMediaField(f)){ const displayUrl = previewImageUrls[f.id] || (typeof v === 'string' && /^(\/|https?:|blob:|data:)/.test(v) ? v : ''); return `<div class="pfield">${label}<div class="mini-image-drop" onclick="pickImage('${f.id}')">${mediaPreviewHtml(fieldKind(f), displayUrl, v, true)}</div></div>`; }
    if(f.type === 'textarea') return `<div class="pfield">${label}<textarea class="mini-textarea" oninput="setPreviewValue('${f.id}',this.value)">${escapeHtml(v)}</textarea></div>`;
    if(f.type === 'number'){ const randomBtn = randomButtonHtml(f); return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="mini-input" type="number" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value)||0)">${randomBtn}</div></div>`; }
    if(f.type === 'slider'){ const min = f.min ?? 0, max = f.max ?? 10, step = f.step ?? 1; return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider"><input type="range" min="${min}" max="${max}" step="${step}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value))"><span class="pfield-slider-val" data-slider-val="${f.id}">${v}</span></div></div></div>`; }
    if(f.type === 'dropdown'){ const opts = (f.options || []).map(o => `<option value="${escapeAttr(o)}" ${String(v)===String(o)?'selected':''}>${escapeHtml(o)}</option>`).join(''); return `<div class="pfield">${label}<select class="pfield-select" onchange="setPreviewValue('${f.id}',this.value)">${opts || `<option value="">${tr('comfy.noOptions')}</option>`}</select></div>`; }
    if(f.type === 'boolean') return `<div class="pfield">${label}<div class="pfield-bool"><div class="pfield-bool-track ${v?'on':''}" onclick="setPreviewValue('${f.id}',!${!!v});this.classList.toggle('on')"><div class="pfield-bool-thumb"></div></div></div></div>`;
...
function miniDeleteButton(node){ return ['prompt','image'].includes(node.type) ? `<button class="mini-delete" type="button" onclick="removeMiniNode('${node.id}')" title="${escapeAttr(tr('common.delete'))}"><i data-lucide="x" class="w-3 h-3"></i></button>` : ''; }
function addMiniNode(type){
    const count = miniTestNodes.filter(n => n.type === type).length;
    miniTestNodes.push({ id:`${type}_${Date.now()}_${Math.random().toString(36).slice(2,5)}`, type, x:42 + count * 26, y:type === 'prompt' ? 86 + count * 170 : 286 + count * 170, text:'', url:'', value:'' });
    renderWorkspaceView();
}
function removeMiniNode(id){ miniTestNodes = miniTestNodes.filter(n => n.id !== id); renderWorkspaceView(); }
function updateMiniNode(id, key, value){ const node = miniTestNodes.find(n => n.id === id); if(node) node[key] = value; }
...
async function pickMiniImage(nodeId){
    ...
    input.onchange = async () => {
        ...
        try { const data = await fetch('/api/comfy/upload', { method:'POST', body:form }).then(r=>r.json()); node.value = data.files?.[0]?.comfy_name || data.files?.[0]?.filename || file.name; }
...
function bindMiniCanvas(){
    ...
    window.onmousemove = e => {
        if(!miniDrag) return;
        if(miniDrag.type === 'pan'){ miniView.x = miniDrag.ox + e.clientX - miniDrag.sx; miniView.y = miniDrag.oy + e.clientY - miniDrag.sy; sync(); }
        else {
            ...
            if(node){ node.x = miniDrag.ox + dx; node.y = miniDrag.oy + dy; }
            else { miniCards[miniDrag.id] = { x: miniDrag.ox + dx, y: miniDrag.oy + dy }; currentConfig.mini_cards = miniCards; }
            const card = world.querySelector(`[data-node="${miniDrag.id}"],[data-card="${miniDrag.id}"]`);
            if(card){ const p = node || miniCards[miniDrag.id]; card.style.left = `${p.x}px`; card.style.top = `${p.y}px`; }
        }
    };
    window.onmouseup = () => { if(miniDrag?.type === 'pan') canvas.classList.remove('is-panning'); const shouldRefresh = miniDrag?.type === 'card'; miniDrag = null; if(shouldRefresh) renderWorkspaceView(); };
}
...
async function pickImage(fieldId){
    ...
    input.onchange = async () => {
        ...
        const form = new FormData();
        form.append('files', file);
        try { const data = await fetch('/api/comfy/upload', { method:'POST', body:form }).then(r=>r.json()); const filename = data.files?.[0]?.comfy_name || data.files?.[0]?.filename || file.name; previewValues[fieldId] = filename; }
...
async function onRun(){
    ...
    try {
        ...
        const res = await fetch(`/api/workflows/${encodeURIComponent(selectedName)}/run`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ fields:runFields, config:currentConfig, client_id:'workflow-test' }) });
    ...
    } catch(e){ alert(e.message || tr('comfy.runFailed')); setStatus(tr('comfy.runFailed')); }
    finally { if(btn){ btn.disabled = false; btn.querySelector('span').textContent = tr('comfy.runTest'); } }
}
function fieldsFromMiniCanvas(){
    ...
    mediaKinds.forEach(kind => {
        ...
        mediaFields.forEach((f, i) => { fields[f.id] = mediaNodes[i]?.value || fields[f.id] || ''; });
    });
    promptFields.forEach(f => { fields[f.id] = prompt || fields[f.id] || ''; });
    return fields;
}
...
async function onUpload(event){
    ...
    try {
        ...
        let workflow;
        try { workflow = JSON.parse(text); } catch { alert(tr('comfy.invalidJson')); return; }
        const baseName = file.name.replace(/\.json$/i, '');
        const inputName = prompt(tr('comfy.namePrompt'), baseName);
        if(!inputName) return;
        const data = await fetch('/api/workflows', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name:inputName, workflow }) });
...
async function onSave(){
    if(!selectedName || !currentConfig) return;
    for(const f of currentConfig.fields){ if(!f.name || !f.name.trim()){ alert(tf('comfy.saveMissingName', {field:f.input})); return; } }
    setStatus(tr('comfy.saving'));
    try {
        const res = await fetch(`/api/workflows/${encodeURIComponent(selectedName)}/config`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(currentConfig) });
...
async function onDelete(){
    ...
    try {
        ...
        selectedName = ''; currentWorkflow = null; currentConfig = null;
        renderEditor(); renderPreview(); renderWorkspaceView();
        await loadList();
