...
const canvasArrangeWrap = document.getElementById('canvasArrangeWrap');
const canvasPromptExpandModal = document.getElementById('canvasPromptExpandModal');
const canvasPromptExpandInput = document.getElementById('canvasPromptExpandInput');
const canvasPromptExpandTitle = document.getElementById('canvasPromptExpandTitle');
const canvasPromptExpandClose = document.getElementById('canvasPromptExpandClose');
let canvasPromptExpandTarget = null;
...
let localChatModels = [];
const MS_GEN_MODELS = {
    zimage:    { label: 'ZImage',     modelId: 'Tongyi-MAI/Z-Image-Turbo',            supportsImage: false, endpoint: '/api/ms/generate'     },
...
function worldToScreen(worldX, worldY){
    const rect = board.getBoundingClientRect();
    return { x: worldX * viewport.scale + viewport.x + rect.left, y: worldY * viewport.scale + viewport.y + rect.top };
}
...
function renderMinimap(){
    ...
    canvasArrangeWrap?.classList.toggle('visible', selected.size > 1);
...
async function saveCanvas(){
    ...
    try {
        const res = await fetch(`/api/canvas/canvases/${canvas.id}`, {
            method:'PUT',
        ...
        if(res.status === 409){
            ...
            console.warn('[Canvas] save conflict; local changes were kept and not overwritten.', {remoteUpdatedAt:data.detail?.updated_at || data.updated_at || remote?.updated_at || 0});
            return;
        }
...
async function loadCanvasList(openFirst=true){
    try {
        const res = await fetch('/api/canvas/canvases');
...
async function loadTrashList(){
    try {
        const res = await fetch('/api/canvas/canvases/trash');
...
async function patchCanvasMeta(id, patch){
    ...
    renderCanvasList();
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}/meta`, {
            method:'PATCH',
            headers:{'Content-Type':'application/json'},
...
async function touchCanvasOpened(id){
    if(!id) return null;
    try {
        const res = await fetch(`/api/canvas/canvases/${encodeURIComponent(id)}/touch`, {method:'POST'});
...
function openSmartCanvasPage(id){
    if(!id) return;
    window.location.href = `/smart-canvas-core/smart-canvas.html?id=${encodeURIComponent(id)}&v=2026.05.22.1`;
}
...
async function setCanvasIcon(id, icon, event){
    ...
    try {
        ...
        if(!target) {
            const data = await fetch(`/api/canvas/canvases/${id}`).then(r => r.json());
            target = data.canvas;
        }
        target.icon = icon || 'layers';
        const res = await fetch(`/api/canvas/canvases/${id}`, {
            method:'PUT',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
                title:target.title,
...
async function openCanvas(id){
    ...
    try {
        ...
        rememberLastCanvasId(canvas.id);
...
async function checkRemoteCanvasVersion(){
    ...
    remoteSyncBusy = true;
    try {
        const res = await fetch(`/api/canvas/canvases/${canvas.id}/meta`);
...
async function deleteCanvas(id, event){
    ...
    try {
        const res = await fetch(`/api/canvas/canvases/${id}`, {method:'DELETE'});
...
async function restoreCanvas(id, event){
    ...
    setStatus('Restoring...');
    try {
        const res = await fetch(`/api/canvas/canvases/${id}/restore`, {method:'POST'});
...
async function purgeCanvas(id, event){
    ...
    setStatus('Deleting...');
    try {
        const res = await fetch(`/api/canvas/canvases/${id}/purge`, {method:'DELETE'});
...
async function runMsGenNode(nodeId, opts={}){
    ...
    try {
        ...
        const submitMs = async () => {
            let apiBody;
            if(modelKey === 'zimage'){
                apiBody = { prompt, model: msModel.modelId, resolution: `${width}x${height}`, client_id: CLIENT_ID };
            } else if(modelKey === 'qwen_edit'){
                apiBody = { prompt, model: msModel.modelId, image_urls: imageUrls, resolution: `${width}x${height}`, client_id: CLIENT_ID };
...
function pendingTaskTypeLabel(pending){
    if(pending?.canvasTaskType === 'video') return '视频任务';
    if(pending?.canvasTaskType === 'comfy') return 'ComfyUI 任务';
    if(pending?.canvasTaskType === 'online-image') return '图片任务';
    return '生成任务';
}
function pendingTaskIdLabel(pending){
    const id = String(pending?.canvasTaskId || pending?.recoverTaskId || '').trim();
    if(!id) return '正在等待任务 ID';
    return `ID：${id.length > 24 ? `${id.slice(0, 12)}…${id.slice(-8)}` : id}`;
}
function renderPendingOutput(pending){
    if(pending?.cancelled){
        const msg = pending.error || '已停止本地等待';
        return `<div class="output-img-wrap loading-wrap task-state-card is-cancelled" data-pending-id="${escapeAttr(pending.id)}"${pendingOutputStyle(pending)}>
            <span class="output-time-pill cancelled">已停止</span>
            <div class="output-task-state">
                <div class="output-task-status"><span class="task-status-dot is-cancelled"></span><strong>任务已停止</strong></div>
                <div class="output-task-kind">${escapeHtml(pendingTaskTypeLabel(pending))}</div>
                <div class="output-task-id" title="${escapeAttr(pending.canvasTaskId || '')}">${escapeHtml(pendingTaskIdLabel(pending))}</div>
                <div class="output-task-message" title="${escapeAttr(msg)}">${escapeHtml(msg)}</div>
            </div>
    ...
    const queued = pending?.canvasTaskStatus === 'queued';
    const waiting = Boolean(pending?.error && !stopping);
    const stateClass = stopping ? 'is-stopping' : waiting ? 'is-waiting' : queued ? 'is-queued' : 'is-running';
    const stateText = stopping ? '正在停止' : waiting ? '等待恢复' : queued ? '排队中' : '正在生成';
    const canCancel = Boolean(pending?.canvasTaskId) && !stopping;
    const taskMessage = pending?.error || pending?.statusMessage || (queued ? '任务已提交，正在等待执行' : '任务正在后台执行');
    return `<div class="output-img-wrap loading-wrap task-state-card ${stateClass}" data-pending-id="${escapeAttr(pending.id)}"${pendingOutputStyle(pending)}>
        <span class="output-time-pill running">${formatRunDuration(nowMs() - Number(pending.startedAt || nowMs()))}</span>
        <div class="output-task-state">
            <div class="output-task-status"><span class="task-status-dot ${stateClass}"></span><strong>${stateText}</strong></div>
            <div class="output-task-kind">${escapeHtml(pendingTaskTypeLabel(pending))}</div>
            <div class="output-task-id" title="${escapeAttr(pending.canvasTaskId || '')}">${escapeHtml(pendingTaskIdLabel(pending))}</div>
            <div class="output-task-message" title="${escapeAttr(taskMessage)}">${escapeHtml(taskMessage)}</div>
            <button class="output-task-cancel" type="button" ${canCancel ? '' : 'disabled'}><i data-lucide="square"></i><span>${stopping ? '停止中…' : (pending?.canvasTaskId ? '停止任务' : '等待提交')}</span></button>
        </div>
        <button class="output-del" title="${tr('common.delete')}">×</button>
    </div>`;
}
...
function canvasPromptExpandButtonHtml(label='放大'){
    return `<button type="button" class="prompt-expand-trigger" data-prompt-expand="1" title="放大 PROMPT" aria-label="放大 PROMPT"><i data-lucide="maximize-2"></i><span>${label}</span></button>`;
}
function bindCanvasPromptExpandTrigger(control, label='PROMPT'){
    if(!control || control.dataset.promptExpandBound === '1') return;
    control.dataset.promptExpandBound = '1';
    const wrap = control.closest('.prompt-editor, .loop-field, .llm-body, .llm-node-pane, .llm-chat-pane, .minimax-prompt-field, .pr-wrapper') || control.parentElement;
    if(!wrap || wrap.querySelector(':scope > .prompt-expand-trigger')) return;
    const trigger = document.createElement('button');
    trigger.type = 'button';
    trigger.className = 'prompt-expand-trigger';
    trigger.title = '放大 PROMPT';
    trigger.setAttribute('aria-label', '放大 PROMPT');
    trigger.innerHTML = '<i data-lucide="maximize-2"></i><span>放大</span>';
    trigger.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); });
    trigger.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); openCanvasPromptExpand(control, label); });
    wrap.appendChild(trigger);
    if(window.lucide) lucide.createIcons();
}
function openCanvasPromptExpand(control, label='PROMPT'){
    if(!canvasPromptExpandModal || !canvasPromptExpandInput || !control) return;
    canvasPromptExpandTarget = control;
    canvasPromptExpandTitle.textContent = label;
    canvasPromptExpandInput.value = control.value ?? control.innerText ?? '';
    canvasPromptExpandModal.hidden = false;
    document.body.classList.add('canvas-prompt-expanded');
    requestAnimationFrame(() => canvasPromptExpandInput.focus());
}
function closeCanvasPromptExpand(){
    if(!canvasPromptExpandModal) return;
    const target = canvasPromptExpandTarget;
    if(target){
        const value = canvasPromptExpandInput.value;
        if('value' in target) target.value = value;
        else target.innerText = value;
        target.dispatchEvent(new Event('input', {bubbles:true}));
        target.dispatchEvent(new Event('change', {bubbles:true}));
    }
    canvasPromptExpandTarget = null;
    canvasPromptExpandModal.hidden = true;
    document.body.classList.remove('canvas-prompt-expanded');
}
function initCanvasPromptExpand(){
    canvasPromptExpandClose?.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); closeCanvasPromptExpand(); });
    canvasPromptExpandModal?.addEventListener('click', e => { if(e.target?.dataset?.canvasPromptExpandClose === '1') closeCanvasPromptExpand(); });
    canvasPromptExpandInput?.addEventListener('keydown', e => { if(e.key === 'Escape') closeCanvasPromptExpand(); });
    canvasPromptExpandModal?.querySelector('.canvas-prompt-expand-card')?.addEventListener('click', e => e.stopPropagation());
}
initCanvasPromptExpand();
...
function renderNode(node){
    ...
    if(node.type === 'group') {
        ...
        body.innerHTML = `<div class="text-[11px] text-gray-400 mb-2">${text}</div><div style="display:flex;gap:4px;flex-wrap:wrap"><button onclick="arrangeGroupFromButton('${node.id}','vertical',event)" class="text-gray-500 hover:text-blue-500 text-[11px] flex items-center gap-1" title="垂直排序"><i data-lucide="align-vertical" class="w-3.5 h-3.5"></i>垂直</button><button onclick="arrangeGroupFromButton('${node.id}','horizontal',event)" class="text-gray-500 hover:text-blue-500 text-[11px] flex items-center gap-1" title="水平排序"><i data-lucide="align-horizontal" class="w-3.5 h-3.5"></i>水平</button><button onclick="arrangeGroupFromButton('${node.id}','grid',event)" class="text-gray-500 hover:text-blue-500 text-[11px] flex items-center gap-1" title="宫格排序"><i data-lucide="layout-grid" class="w-3.5 h-3.5"></i>宫格</button></div>`;
    ...
    if(node.type === 'output') {
        ...
        body.onwheel = e => {
            // 输出网格只有"能继续按当前方向滚动"时才消费 wheel；到顶/底则放行,让画布缩放接管
            const deltaY = e.deltaY;
            const scrollTop = body.scrollTop || 0;
            const maxScroll = (body.scrollHeight || 0) - (body.clientHeight || 0);
            const canScrollDown = deltaY > 0 && scrollTop < maxScroll - 1;
            const canScrollUp = deltaY < 0 && scrollTop > 1;
            if(canScrollDown || canScrollUp) e.stopPropagation();
        };
        body.querySelectorAll('.output-img-wrap').forEach(wrap => bindOutputWrap(wrap, node));
    }
    el.appendChild(body);
    el.querySelectorAll('textarea').forEach(control => {
        const cls = String(control.className || '');
        const isPromptControl = node.type === 'prompt' || node.type === 'loop' || /prompt|system|instruction|chat-input|llm-input|ltx|comfy|rh-param|minimax/i.test(cls + ' ' + (control.placeholder || ''));
        if(isPromptControl) bindCanvasPromptExpandTrigger(control, node.type === 'llm' ? 'LLM PROMPT' : 'PROMPT');
    });
    el.querySelectorAll('.loop-variable-editor, .pr-prompt-area').forEach(control => bindCanvasPromptExpandTrigger(control, 'PROMPT'));
...
function bindOutputWrap(wrap, node){
    ...
    const taskCancel = wrap.querySelector('.output-task-cancel');
    ...
    if(del){
        ...
        del.onclick = e => {
            ...
            if(pid){
                ...
                if(pending?.canvasTaskId && !pending.cancelled && !pending.failed){
                    cancelPendingOutputTask(node.id, pid);
                    return;
                }
    ...
    if(taskCancel){
        taskCancel.onmousedown = e => e.stopPropagation();
        taskCancel.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const pid = wrap.dataset.pendingId;
            if(pid) cancelPendingOutputTask(node.id, pid);
        };
    }
}
...
function renderLoopBody(node){
    ...
    if(variable) {
        ...
        variable.onwheel = e => {
            // 循环节点变量输入框只有"能继续按当前方向滚动"时才消费 wheel；到顶/底则放行,让画布缩放接管
            const deltaY = e.deltaY;
            const scrollTop = variable.scrollTop || 0;
            const maxScroll = (variable.scrollHeight || 0) - (variable.clientHeight || 0);
            const canScrollDown = deltaY > 0 && scrollTop < maxScroll - 1;
            const canScrollUp = deltaY < 0 && scrollTop > 1;
            if(canScrollDown || canScrollUp) e.stopPropagation();
        };
...
function bindScrollableText(el){
    ...
    el.addEventListener('wheel', e => {
        ...
        const canScrollDown = deltaY > 0 && scrollTop < maxScroll - 1;
        const canScrollUp = deltaY < 0 && scrollTop > 1;
        // 文本框到达当前方向边界时放行，让画布级缩放处理该 wheel。
        if(canScrollDown || canScrollUp) e.stopPropagation();
...
function miniMaxEnsureSegment(node){
    ...
    node.segments.forEach((seg, index) => {
        ...
        seg.refs = miniMaxUniqueRefs(migrated);
...
function miniMaxRefsForSegment(node, seg){
    ...
    return miniMaxUniqueRefs(upstream);
}
function miniMaxAssertReferenceLimits(refs){
...
const CLASSIC_VIDEO_POLL_INTERVAL = 5000;
const CLASSIC_VIDEO_POLL_MAX = 360;
const stoppedClassicCanvasTaskIds = new Set();
function classicCanvasTaskStoppedError(){
    const error = new Error('任务已由用户停止');
    error.canvasTaskCancelled = true;
    return error;
}
async function pollClassicCanvasVideo(providerId, taskId, options={}){
    for(let i = 0; i < CLASSIC_VIDEO_POLL_MAX; i++){
        if(stoppedClassicCanvasTaskIds.has(taskId)) throw classicCanvasTaskStoppedError();
        const cascadeTargetId = cascadeTargetIdFromOptions(options);
        if(cascadeTargetId) ensureCascadeActive(cascadeTargetId);
        await sleep(CLASSIC_VIDEO_POLL_INTERVAL);
        if(stoppedClassicCanvasTaskIds.has(taskId)) throw classicCanvasTaskStoppedError();
        const res = await cascadeFetch(`/api/canvas-video/${encodeURIComponent(taskId)}?provider_id=${encodeURIComponent(providerId || '')}`, {}, {cascadeTargetId});
        if(!res.ok) throw new Error(await responseErrorMessage(res, tr('canvas.videoFailed')));
        const data = await res.json();
        updateClassicPendingTaskRuntime(taskId, data.status || 'running', data.message || '');
        if(data.status === 'succeeded') { stoppedClassicCanvasTaskIds.delete(taskId); return data; }
        if(['cancelled','canceled'].includes(data.status)) throw classicCanvasTaskStoppedError();
        if(data.status === 'interrupted') { const error = new Error(data.error || data.detail || '本地服务重启，视频任务轮询已中断'); error.videoTaskTerminal = true; throw error; }
        if(data.status === 'failed') { const error = new Error(data.error || data.detail || tr('canvas.videoFailed')); error.videoTaskTerminal = true; throw error; }
    }
    throw new Error('视频生成仍在云端执行，任务 ID 已保存，可刷新画布后继续查询');
}
async function resumeClassicCanvasVideoTask(out, pending){
    if(!out || !pending?.canvasTaskId || pending.querying) return;
    pending.querying = true;
    refreshNodes([out.id]);
    try {
        const result = await pollClassicCanvasVideo(pending.providerId, pending.canvasTaskId, {cascadeTargetId:pending.cascadeTargetId || ''});
        ...
        appendOutputImages(out, outputUrls, run.refs?.[0], [{runMs, run, kind:'video'}]);
        const gen = nodes.find(node => node.id === run?.node?.id);
        if(gen){ mergeGeneratedOutputs(gen, outputUrls, Boolean(pending.appendGenerated)); gen.runStatus = 'done'; gen.runError = ''; gen.running = false; }
        addGenerationLog({run, outputs:outputUrls, runMs});
        refreshRunNodes(gen, out);
        scheduleSave();
    } catch(err){
        const latest = pendingById(out, pending.id);
        if(!latest) return;
        if(err?.canvasTaskCancelled){
            latest.querying = false;
            latest.cancelling = false;
            latest.cancelled = true;
            latest.error = latest.error || '任务已由用户停止';
            refreshNodes([out.id]);
            scheduleSave();
            return;
        }
        latest.querying = false;
        latest.error = err.message || tr('canvas.videoFailed');
        if(err?.videoTaskTerminal){
            out._pending = (out._pending || []).filter(item => item.id !== pending.id);
            const gen = nodes.find(node => node.id === pending.run?.node?.id);
            if(gen){ gen.runStatus = 'failed'; gen.runError = latest.error; gen.running = false; }
            addGenerationLog({run:pending.run || {}, outputs:[], runMs:nowMs() - Number(pending.startedAt || nowMs()), error:latest.error});
            refreshRunNodes(gen, out);
        } else refreshNodes([out.id]);
        scheduleSave();
    }
}
async function runVideoNode(nodeId, opts={}){
    ...
    try {
        ...
        if(result?.task_id && result.status === 'submitted'){
            submittedVideoTaskId = result.task_id;
            const pending = pendingById(out, pendingId);
            if(pending){
                pending.canvasTaskType = 'video';
                pending.canvasTaskId = result.task_id;
                pending.providerId = result.provider_id || resolveVideoProviderId(node.apiProvider || 'comfly');
                pending.model = node.model || 'veo3-fast';
                pending.error = '';
                scheduleSave();
                await saveCanvas();
            }
            result = await pollClassicCanvasVideo(result.provider_id || resolveVideoProviderId(node.apiProvider || 'comfly'), result.task_id, {cascadeTargetId});
        }
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending || []).filter(p => p.id !== pendingId);
        const outputUrls = resultMediaUrls(result).map(item => {
        ...
        const savedPending = submittedVideoTaskId ? pendingById(out, pendingId) : null;
        if(err?.canvasTaskCancelled){
            if(savedPending){ savedPending.cancelled = true; savedPending.cancelling = false; savedPending.error = savedPending.error || '任务已由用户停止'; }
            node.runStatus = 'cancelled'; node.runError = savedPending?.error || '任务已停止';
            refreshRunNodes(node, out);
            scheduleSave();
            return;
        }
        if(savedPending && !err?.videoTaskTerminal && !isCascadeAbortError(err)){
            savedPending.error = err.message || tr('canvas.videoFailed');
            savedPending.querying = false;
            node.runStatus = 'waiting'; node.runError = '视频任务 ID 已保存，可刷新后继续查询';
            refreshRunNodes(node, out);
            scheduleSave();
            if(!opts.cascade) alert(node.runError);
            return;
        }
        const meta = collectRunMeta(out, pendingId);
        addGenerationLog({run, outputs:[], runMs:meta.runMs || 0, error:err.message || String(err)});
...
async function miniMaxDynamicParams(node, prompt, refs){
    ...
    const {images, videos, audios} = miniMaxAssertReferenceLimits(refs);
...
async function runMiniMaxNode(nodeId, opts={}){
    ...
    try {
        miniMaxAssertReferenceLimits(refs);
    } catch(error) {
        if(opts.cascade) throw error;
    ...
    if(!opts.cascade) node.running = true;
    refreshRunNodes(node, out);
    const comfyTaskOptions = {cascadeTargetId, pending:pendingById(out, pendingId)};
    try {
        let outputs = [];
        ...
        if(preserveClassicQueuedTaskFailure(node, out, pendingId, err)){
            if(opts.cascade) throw err;
            return;
        }
        if(out) out._pending = (out._pending || []).filter(p => p.id !== pendingId);
...
function requestCascadeStop(targetId, reason=''){
    ...
    nodes.filter(node => node.type === 'output').forEach(out => {
        (out._pending || []).filter(pending => pending.cascadeTargetId === targetId && pending.canvasTaskId && !pending.cancelled && !pending.failed).forEach(pending => {
            cancelPendingOutputTask(out.id, pending.id);
        });
    });
    ...
    try {
        ...
        if(mode === 'text'){
            ...
            if(node.enhanceUpscale){
                images = await runComfyUpscale(enhance.images?.[0], node.enhanceUpscaleRes || 2048, comfyTaskOptions);
            } else {
            ...
            images = node.editUpscale ? await runComfyUpscale(result.images?.[0], node.editUpscaleRes || 2048, comfyTaskOptions) : result.images || [];
        }
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending||[]).filter(p => p.id !== pendingId);
        ...
        const meta = collectRunMeta(out, pendingId);
        addGenerationLog({run, outputs:[], runMs:meta.runMs || 0, error:err.message || String(err)});
        const activePending = pendingById(out, pendingId);
        if(err?.canvasTaskCancelled){
            if(activePending){
                activePending.querying = false;
                activePending.cancelling = false;
                activePending.cancelled = true;
                activePending.canvasTaskStatus = 'cancelled';
                activePending.error = activePending.error || '任务已由用户停止';
            }
            node.running = false;
            node.runStatus = 'cancelled';
            node.runError = activePending?.error || err.message || '任务已停止';
            refreshRunNodes(node, out);
            scheduleSave();
            if(opts.cascade) throw err;
            return;
        ...
        if(isCascadeAbortError(err)){
            if(activePending?.canvasTaskId){
                activePending.querying = false;
                activePending.canvasTaskStatus = activePending.canvasTaskStatus || 'interrupted';
                activePending.error = activePending.error || '停止请求处理中，任务 ID 已保留';
        ...
        if(activePending?.canvasTaskId && !err?.comfyTaskTerminal){
            activePending.querying = false;
            activePending.canvasTaskStatus = 'interrupted';
            activePending.error = err.message || '任务仍在后台执行，等待恢复查询';
            node.running = false;
            node.runStatus = 'queued';
            node.runError = activePending.error;
            refreshRunNodes(node, out);
            scheduleSave();
            if(opts.cascade) throw err;
            return;
...
function preserveClassicQueuedTaskFailure(node, out, pendingId, error){
    const pending = pendingById(out, pendingId);
    if(!pending?.canvasTaskId) return false;
    if(error?.canvasTaskCancelled){
        pending.querying = false;
        pending.cancelling = false;
        pending.cancelled = true;
        pending.canvasTaskStatus = 'cancelled';
        pending.error = pending.error || '任务已由用户停止';
        if(node){ node.running = false; node.runStatus = 'cancelled'; node.runError = pending.error; }
    } else if(isCascadeAbortError(error) || !error?.comfyTaskTerminal){
        pending.querying = false;
        pending.canvasTaskStatus = 'interrupted';
        pending.error = pending.error || error?.message || '任务仍在后台执行，等待恢复查询';
        if(node){ node.running = false; node.runStatus = 'queued'; node.runError = pending.error; }
    } else return false;
    refreshRunNodes(node, out);
    scheduleSave();
    return true;
}
async function cancelPendingOutputTask(outId, pendingId){
    const out = nodes.find(node => node.id === outId && node.type === 'output');
    const pending = pendingById(out, pendingId);
    if(!out || !pending || pending.cancelling || pending.cancelled) return;
    const taskId = String(pending.canvasTaskId || '').trim();
    if(!taskId){
        pending.error = '任务尚未返回 ID，暂时无法停止';
        refreshNodes([out.id]);
        return;
    }
    ...
    scheduleSave();
    try {
        let endpoint = `/api/canvas/tasks/${encodeURIComponent(taskId)}/cancel`;
        let body = {};
        if(pending.canvasTaskType === 'online-image') endpoint = `/api/canvas-image-tasks/${encodeURIComponent(taskId)}/cancel`;
        else if(pending.canvasTaskType === 'video') { endpoint = `/api/canvas-video/${encodeURIComponent(taskId)}/cancel`; body = {provider_id:pending.providerId || ''}; }
        else if(pending.canvasTaskType === 'comfy') endpoint = `/api/canvas-comfy-tasks/${encodeURIComponent(taskId)}/cancel`;
        const response = await fetch(endpoint, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)});
        let data = {};
        try { data = await response.json(); } catch(_error) {}
        if(!response.ok) throw new Error(data.detail || data.error || '停止任务失败');
        stoppedClassicCanvasTaskIds.add(taskId);
        pending.cancelling = false;
        pending.cancelled = true;
        pending.failed = false;
        pending.canvasTaskStatus = 'cancelled';
        pending.error = data.message || data.error || '任务已停止';
        const gen = nodes.find(node => node.id === pending.run?.node?.id);
        if(gen){ gen.running = false; gen.runStatus = 'cancelled'; gen.runError = pending.error; }
        refreshRunNodes(gen, out);
        scheduleSave();
    } catch(error){
        pending.cancelling = false;
        pending.error = error.message || '停止任务失败';
        refreshNodes([out.id]);
        scheduleSave();
        alert(pending.error);
    }
...
function updateClassicPendingTaskRuntime(taskId, status, message=''){
    const found = findPendingTask(taskId);
    if(!found) return;
    const normalized = String(status || '').toLowerCase();
    const changed = found.pending.canvasTaskStatus !== normalized || (message && found.pending.statusMessage !== message);
    found.pending.canvasTaskStatus = normalized;
    if(message) found.pending.statusMessage = message;
    if(changed){ refreshNodes([found.out.id]); scheduleSave(); }
}
...
async function waitCanvasComfyTaskResult(taskId, options={}){
    ...
    while(true){
        if(stoppedClassicCanvasTaskIds.has(taskId) || options?.pending?.cancelled) throw classicCanvasTaskStoppedError();
        const cascadeTargetId = cascadeTargetIdFromOptions(options);
        if(cascadeTargetId) ensureCascadeActive(cascadeTargetId);
        ...
        const data = await res.json();
        updateClassicPendingTaskRuntime(taskId, data.status || 'running', data.error || data.message || '');
        if(data.status === 'succeeded') { stoppedClassicCanvasTaskIds.delete(taskId); return data.result || {}; }
        if(['cancelled','canceled'].includes(data.status)) throw classicCanvasTaskStoppedError();
        if(['failed','interrupted'].includes(data.status)) { const error = new Error(data.error || actionFailed('canvas.comfyGenerate')); error.comfyTaskTerminal = true; throw error; }
        await sleep(1600);
    }
}
async function runQueuedComfyGenerate(payload, options={}){
    ...
    if(pending && taskId){
        ...
        pending.providerId = 'comfyui';
        pending.canvasTaskStatus = String(task.status || 'queued').toLowerCase();
        pending.error = '';
        const out = findOutputByPendingId(pending.id);
        if(out) refreshNodes([out.id]);
        scheduleSave();
        await saveCanvas();
    }
    return waitCanvasComfyTaskResult(taskId, options);
}
...
async function pollCanvasImageTask(taskId, options={}){
    ...
    try {
        while(true){
            ...
            if(found.pending?.cancelled || stoppedClassicCanvasTaskIds.has(taskId)) return 'aborted';
            ...
            const data = await res.json();
            updateClassicPendingTaskRuntime(taskId, data.status, data.message || '');
            if(data.status === 'succeeded'){
...
async function waitCanvasImageTaskResult(taskId, options={}){
    ...
    while(true){
        ...
        if(['failed','cancelled','interrupted'].includes(data.status)) throw new Error(data.error || tr('canvas.generationFailed'));
        await sleep(1800);
    }
}
...
async function resumeClassicCanvasComfyTask(out, pending){
    if(!out || !pending?.canvasTaskId || pending.querying || pending.cancelled) return;
    pending.querying = true;
    refreshNodes([out.id]);
    try {
        const result = await waitCanvasComfyTaskResult(pending.canvasTaskId, {cascadeTargetId:pending.cascadeTargetId || '', pending});
        const outputs = comfyResultOutputs(result);
        if(!outputs.length){
            const error = new Error(noReturnedImage('canvas.comfyGenerate'));
            error.comfyTaskTerminal = true;
            throw error;
        }
        const run = pending.run || {};
        run.request = requestMetaFromResult(result);
        const runMs = nowMs() - Number(pending.startedAt || nowMs());
        const firstKind = mediaKindForOutputItem(outputs[0]);
        out._pending = (out._pending || []).filter(item => item.id !== pending.id);
        appendOutputImages(out, outputs, run.refs?.[0], [{runMs, run, kind:firstKind}]);
        const gen = nodes.find(node => node.id === run?.node?.id);
        if(gen){
            mergeGeneratedOutputs(gen, outputs, Boolean(pending.appendGenerated));
            if(gen.type === 'minimax'){
                const segmentId = run?.node?.selectedSegmentId || gen.selectedSegmentId;
                const segment = (gen.segments || []).find(item => item.id === segmentId) || miniMaxSelectedSegment(gen);
                outputs.forEach(item => miniMaxSetSegmentResult(gen, segment, item));
            }
            gen.runStatus = 'done';
            gen.runError = '';
            gen.running = false;
        ...
        if(error?.canvasTaskCancelled){
            latest.cancelling = false;
            latest.cancelled = true;
            latest.canvasTaskStatus = 'cancelled';
            latest.error = latest.error || '任务已由用户停止';
            refreshNodes([out.id]);
            scheduleSave();
            return;
        }
        latest.canvasTaskStatus = error?.comfyTaskTerminal ? 'failed' : 'interrupted';
        latest.error = error.message || actionFailed('canvas.comfyGenerate');
        if(error?.comfyTaskTerminal){
            out._pending = (out._pending || []).filter(item => item.id !== pending.id);
            const gen = nodes.find(node => node.id === pending.run?.node?.id);
            if(gen){ gen.runStatus = 'failed'; gen.runError = latest.error; gen.running = false; }
            addGenerationLog({run:pending.run || {}, outputs:[], runMs:nowMs() - Number(pending.startedAt || nowMs()), error:latest.error});
...
function resumeCanvasImageTasks(){
    nodes.filter(n => n.type === 'output').forEach(out => {
        (out._pending || []).forEach(p => {
            if(p.canvasTaskType === 'online-image' && p.canvasTaskId && !p.failed && !p.cancelled) pollCanvasImageTask(p.canvasTaskId, {cascadeTargetId:p.cascadeTargetId || ''});
            if(p.canvasTaskType === 'video' && p.canvasTaskId && !p.failed && !p.cancelled) resumeClassicCanvasVideoTask(out, p);
            if(p.canvasTaskType === 'comfy' && p.canvasTaskId && !p.failed && !p.cancelled) resumeClassicCanvasComfyTask(out, p);
        });
    });
}
...
function finishSelection(){
    ...
    if(!selectionAppend) selected.clear();
    nodesEl.querySelectorAll('.node').forEach(el => {
...
function renderSelectionHub(){
    ...
    selectionHub.classList.remove('open');
    const wrap = document.getElementById('canvasArrangeWrap');
    if(!canvas){ wrap?.classList.remove('visible'); return; }
    // 仅在多选且未成组、且全部是可整理的 image/prompt/loop/generator/output 等可移动节点时,显示「整理选中」工具栏
    const selIds = [...selected];
    if(selIds.length < 2){ wrap?.classList.remove('visible'); return; }
    const selNodes = selIds.map(id => nodes.find(n => n.id === id)).filter(Boolean);
    if(selNodes.length < 2){ wrap?.classList.remove('visible'); return; }
    // 如果选中的全是某个分组的所有成员,交给分组工具栏处理(本工具栏不展示)
    if(selNodes.every(n => n.type === 'group' || n.type === 'promptGroup')){ wrap?.classList.remove('visible'); return; }
    // 计算选区 bbox,用于 arrangeSelectedNodes 计算 box
    const box = nodeBounds(selIds);
    if(!box){ wrap?.classList.remove('visible'); return; }
    const menu = document.createElement('div');
    menu.className = 'hub-arrange-menu';
    menu.hidden = true;
    const items = [
        {key:'grid', icon:'layout-grid', label:'宫格排列'},
        {key:'horizontal', icon:'align-horizontal', label:'水平排列'},
        {key:'vertical', icon:'align-vertical', label:'垂直排列'}
    ];
    items.forEach(item => {
        const itemBtn = document.createElement('button');
        itemBtn.type = 'button';
        itemBtn.className = 'hub-arrange-item';
        itemBtn.dataset.arrangeMode = item.key;
        itemBtn.innerHTML = `<i data-lucide="${item.icon}"></i><span>${item.label}</span>`;
        itemBtn.addEventListener('click', e => {
            e.preventDefault();
            e.stopPropagation();
            arrangeSelectedNodes(item.key, selNodes, box);
            menu.hidden = true;
            canvasArrangeBtn?.setAttribute('aria-expanded', 'false');
        });
    ...
    selectionHub.appendChild(menu);
    wrap?.classList.add('visible');
    canvasArrangeBtn.onclick = e => {
        e.preventDefault();
        e.stopPropagation();
        const open = menu.hidden;
        menu.hidden = !open;
        canvasArrangeBtn?.setAttribute('aria-expanded', String(open));
    };
    canvasArrangeBtn?.setAttribute('aria-expanded', 'false');
    selectionHub.classList.add('open');
    if(window.lucide) lucide.createIcons();
}
// 对未成组的选中节点应用三种排列模式(不创建 group,只调整位置),起点为 box 左上角
function arrangeSelectedNodes(getMode, selNodes, box){
    if(!Array.isArray(selNodes) || selNodes.length < 2) return false;
    const mode = typeof getMode === 'function' ? getMode() : getMode;
    const pad = 18;
    const gap = 4;
    const sizes = selNodes.map(node => {
        const el = nodesEl.querySelector(`.node[data-id="${node.id}"]`);
        const w = el?.offsetWidth || Number(node.w) || 260;
        const h = el?.offsetHeight || Number(node.h) || 180;
        return {node, w:Math.max(40, w), h:Math.max(40, h)};
    });
    const count = sizes.length;

    // 可用空间:视口世界尺寸(等价于 shell.clientWidth/scale × shell.clientHeight/scale)
    const scale = viewport.scale || 1;
    const availW = Math.max(1, (board.clientWidth || 0) / scale);
    const availH = Math.max(1, (board.clientHeight || 0) / scale);
    ...
    // 每列 colW 取该列最大节点宽,每行 rowH 取该行最大节点高
...
function pushUndo(){
    if(!canvas) return;
    undoStack.push(canvasHistorySnapshot());
    if(undoStack.length > UNDO_MAX) undoStack.shift();
    redoStack.length = 0;
}
function performUndo(){
    if(!canvas || !undoStack.length) return;
    redoStack.push(canvasHistorySnapshot());
...
// 普通画布分组整理排列：支持 vertical / horizontal / grid 三种模式，节点间隙极小（4px）
function arrangeGroupMembers(group, mode='grid'){
    if(!group || group.type !== 'group') return false;
    const memberIds = Array.isArray(group.items) ? group.items.slice() : [];
    const memberNodes = memberIds.map(id => nodes.find(n => n.id === id)).filter(Boolean);
    if(!memberNodes.length) return false;
    pushUndo();
    const sizes = memberNodes.map(node => {
        const w = Math.max(60, Number(node.w) || 120);
        const h = Math.max(40, Number(node.h) || 120);
        return {node, w, h};
    });
    const count = sizes.length;
    const pad = 18;
    ...
    const originY = (Number(group.y) || 0) + pad;
    sizes.forEach((s, i) => {
        const c = i % cols, r = Math.floor(i / cols);
        s.node.x = Math.round(originX + colX[c] + (colW[c] - s.w) / 2);
        s.node.y = Math.round(originY + rowY[r] + (rowH[r] - s.h) / 2);
    });
    const totalW = colW.reduce((a, b) => a + b, 0) + gap * Math.max(0, cols - 1) + pad * 2;
    const totalH = rowH.reduce((a, b) => a + b, 0) + gap * Math.max(0, rows - 1) + pad * 2;
    if(group.w !== totalW || group.h !== totalH){
        group.w = Math.max(180, Math.round(totalW));
        group.h = Math.max(120, Math.round(totalH));
    }
    render();
    scheduleSave();
    return true;
}
// inline onclick 入口：阻止冒泡 + 调用整理函数
function arrangeGroupFromButton(groupId, mode, event){
    event?.preventDefault();
    event?.stopPropagation();
    const group = nodes.find(n => n.id === groupId);
    if(!arrangeGroupMembers(group, mode)){
        toast('分组内没有可整理的节点');
        return;
    }
...
board.onmousedown = e => {
    ...
    if(e.ctrlKey || e.metaKey){
        e.preventDefault();
        startSelection(e, true);
        return;
    }
    // 左键空白 = 框选（点空白因框选面积为 0，finishSelection 自然清空选择）
    startSelection(e);
};
...
board.addEventListener('wheel', e => {
    if(!canvas || !e.ctrlKey || e.target.closest?.('#canvasAssetPanel,#workflowTransferModal')) return;
    e.preventDefault();
    e.stopPropagation();
...
window.addEventListener('blur', () => { isRKeyDown = false; spaceKeyDown = false; setKnifeMode(false); });
