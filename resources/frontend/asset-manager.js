...
function writeLocalCaptionSettings(){
    try {
        localStorage.setItem(LOCAL_CAPTION_SETTINGS_KEY, JSON.stringify({
            ...
            captionPrompt:localCaptionPrompt || L('描述图片','Describe the image'),
...
let localCaptionPrompt = savedLocalCaptionSettings.captionPrompt || L('描述图片','Describe the image');
...
let canvasAssetCenterItems = [];
let canvasAssetCenterProjects = [];
...
let canvasAssetScope = 'all';
let canvasAssetTypeFilter = 'all';
let canvasAssetProjectFilter = 'all';
let canvasAssetShowTrash = false;
...
const CANVAS_MEDIA_EXTS = /\.(png|jpe?g|webp|gif|bmp|avif|svg|heic|tiff?|mp4|webm|mov|m4v|avi|mkv|flv|mp3|wav|flac|ogg|m4a|aac)(\?|#|$)/i;
...
function setStatus(text='准备就绪'){ if(statusEl) statusEl.textContent = text || tr('asset.ready'); }
...
async function apiJson(url, options={}){
    ...
    if(!res.ok) throw new Error(data.detail || data.error || data.message || L('操作失败','Operation failed'));
    return data;
}
const STORAGE_KIND_LABELS = {upload:L('上传素材','Uploaded'), generated:L('生成素材','Generated'), local:L('本地素材','Local')};
async function openStorageSettings(){
    ...
    try {
        ...
        setStatus(err.message || L('加载存储设置失败','Load storage settings failed'));
        renderStorageSettingsModal();
    }
}
...
function syncStorageSettingsInputsToState(){
    ...
    if(captionEl) localCaptionPrompt = captionEl.value || L('描述图片','Describe the image');
...
async function saveStorageSettings(options={}){
    ...
    setStatus(L('偏好设置已保存','Preferences saved'));
}
...
function handleStorageFileGridScroll(event){
    ...
    if(grid.scrollTop + grid.clientHeight >= grid.scrollHeight - 260){
        loadStorageFiles(storageSettingsState.kind, {append:true}).catch(err => setStatus(err.message || L('加载更多图片失败','Load more images failed')));
    }
}
function renderStorageSettingsModal(){
    ...
    const cards = storageSettingsState.loading
        ? `<div class="storage-empty">${L('正在读取目录...','Reading directory...')}</div>`
        ...
        `).join('') + (storageSettingsState.hasMore || storageSettingsState.loadingMore
            ? `<div class="storage-load-more">${storageSettingsState.loadingMore ? L('继续加载中...','Loading more...') : L(`已加载 ${loadedCount} / ${storageSettingsState.total || loadedCount}，向下滚动继续`,`Loaded ${loadedCount} / ${storageSettingsState.total || loadedCount}, scroll to continue`)}</div>`
            : `<div class="storage-load-more done">${L(`已加载全部 ${loadedCount} 张`,`All ${loadedCount} loaded`)}</div>`)
        : `<div class="storage-empty">${L('这个目录里暂时没有图片','No images in this directory yet')}</div>`;
    ...
    const prefsBody = `
        <div class="asset-pref-section">
            <div class="asset-pref-title"><i data-lucide="wand-sparkles"></i><span>${L('基础偏好','Basic preferences')}</span></div>
            <div class="asset-pref-grid">
                <label class="storage-dir-row">
                    <span>${L('平台','Platform')}</span>
        ...
        <div class="asset-pref-section">
            <button class="asset-pref-fold ${captionOpen ? 'open' : ''}" type="button" data-pref-editor="caption">
                <span><i data-lucide="message-square-text"></i><b>${L('反推提示词','Caption reverse')}</b></span>
                <em>${escapeHtml(localCaptionPrompt || L('描述图片','Describe the image'))}</em>
                <i data-lucide="${captionOpen ? 'chevron-up' : 'chevron-down'}"></i>
            </button>
            ${captionOpen ? `
                <div class="asset-pref-editor">
                    <label class="storage-dir-row">
                        <span>${L('反推提示词','Caption reverse')}</span>
                        <textarea id="prefCaptionPrompt" placeholder="${L('描述图片','Describe the image')}">${escapeHtml(localCaptionPrompt || L('描述图片','Describe the image'))}</textarea>
                    </label>
                    <div class="asset-pref-editor-actions">
                        <button class="asset-btn primary" type="button" data-pref-editor-save="caption"><i data-lucide="save"></i><span>${L('保存','Save')}</span></button>
                    </div>
                </div>
            ` : ''}
            <button class="asset-pref-fold ${classifyOpen ? 'open' : ''}" type="button" data-pref-editor="classify">
                <span><i data-lucide="tags"></i><b>${L('智能分类提示词','Smart classification prompt')}</b></span>
                <em>${L('编辑分类规则和追加要求','Edit classification rules and extra requirements')}</em>
                <i data-lucide="${classifyOpen ? 'chevron-up' : 'chevron-down'}"></i>
            </button>
            ${classifyOpen ? `
                <div class="asset-pref-editor">
                    <label class="storage-dir-row">
                        <span>${L('智能分类追加要求','Extra classification requirements')}</span>
                        <textarea id="prefClassifyPrompt" placeholder="${L('可追加分类要求，例如：增加家装风格、镜头焦段、商业用途、人物年龄段、产品材质。','Optional extra classification requirements, e.g. interior style, focal length, commercial use, age group, product material.')}">${escapeHtml(localClassifyPrompt || '')}</textarea>
                    </label>
                    <label class="storage-dir-row">
                        <span>${L('智能分类规则','Smart classification rules')} <button class="asset-inline-link" type="button" data-class-rule-reset>${L('恢复内置规则','Reset built-in rules')}</button></span>
                        <textarea id="prefClassificationRulePrompt" class="asset-pref-rule-textarea" placeholder="${L('这里显示内置分类提示词，可按你的素材体系编辑后保存。','Shows the built-in classification prompt; edit and save per your asset system.')}">${escapeHtml(storageSettingsState.classificationPrompt || storageSettingsState.defaultClassificationPrompt || '')}</textarea>
                    </label>
                    <div class="asset-pref-editor-actions">
                        <button class="asset-btn primary" type="button" data-pref-editor-save="classify"><i data-lucide="save"></i><span>${L('保存','Save')}</span></button>
                    </div>
                </div>
            ` : ''}
        </div>
    ...
    const manageBody = `
        <div class="asset-pref-section">
            <div class="asset-pref-title"><i data-lucide="folder-cog"></i><span>${L('保存目录','Save directory')}</span></div>
            <div class="storage-dir-grid">${rows}</div>
            <div class="storage-settings-actions inline">
                <button class="asset-btn primary" type="button" data-storage-save><i data-lucide="save"></i><span>${L('保存目录','Save directory')}</span></button>
            </div>
        </div>
        <div class="storage-file-head">
            <div class="storage-tabs">${tabs}</div>
            <div class="storage-file-actions">
                <button class="asset-btn" type="button" data-storage-select-all ${storageSettingsState.items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                <button class="asset-btn danger" type="button" data-storage-delete ${selectedCount ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${L(`删除 ${selectedCount || ''}`,`Delete ${selectedCount || ''}`)}</span></button>
            </div>
        </div>
    ...
    overlay.innerHTML = `
        <div class="storage-settings-modal">
            <div class="storage-settings-head">
                <div>
                    <strong>${L('偏好设置','Preferences')}</strong>
                    <span>${L('分开管理默认反推设置和素材目录文件。','Separately manage default caption settings and asset directory files.')}</span>
                </div>
                <button type="button" data-storage-close><i data-lucide="x"></i></button>
            </div>
            <div class="asset-pref-tabs">
                <button class="${activePrefTab === 'prefs' ? 'active' : ''}" type="button" data-pref-tab="prefs"><i data-lucide="sliders-horizontal"></i><span>${L('偏好设置','Preferences')}</span></button>
                <button class="${activePrefTab === 'manage' ? 'active' : ''}" type="button" data-pref-tab="manage"><i data-lucide="images"></i><span>${L('素材管理','Asset management')}</span></button>
            </div>
...
async function deleteSelectedStorageFiles(){
    ...
    if(!confirm(L(`确认删除 ${selected.length} 张图片？此操作会删除磁盘文件。`,`Delete ${selected.length} images? This removes files from disk.`))) return;
    ...
    setStatus(L(`已删除 ${data.removed || 0} 个文件`,`Deleted ${data.removed || 0} files`));
}
function formatDate(value){
    const num = Number(value || 0);
    if(!num) return L('未知','Unknown');
    try { return new Date(num).toLocaleString('zh-CN', {hour12:false}); }
    catch(e) { return L('未知','Unknown'); }
}
...
function assetLibraries(){
    return Array.isArray(assetLibrary.libraries) && assetLibrary.libraries.length
        ...
        : [{id:'default', name:L('默认资产库','Default asset library'), categories:assetLibrary.categories || []}];
}
...
function workflowCategories(){
    ...
    assetLibraries().forEach(lib => {
        (lib.categories || []).filter(cat => (cat.type || '') === 'workflow').forEach(cat => {
            cats.push({...cat, __libraryId:lib.id, __libraryName:lib.name || L('资产库','Asset library')});
        });
    });
    return cats;
}
...
function assetViewTitle(){
    ...
    if(entry) return entry.tag || L('智能分类','Smart classify');
    return activeAssetCategory()?.name || L('图片资产','Image assets');
}
function assetViewSubtitle(items){
    const libName = activeAssetLibrary()?.name || L('资产库','Asset library');
    const entry = activeAssetClassEntry();
    if(entry) return L(`${libName} / 智能分类：${entry.label} / ${items.length} 个素材`,`${libName} / Smart: ${entry.label} / ${items.length} assets`);
    return L(`${libName} / ${items.length} 个素材`,`${libName} / ${items.length} assets`);
}
...
function assetCountForLibrary(lib){
    return (lib?.categories || [])
        ...
        .reduce((sum, cat) => sum + mediaAssetItems(cat.items).length, 0);
}
function promptLibraries(){
    const libs = Array.isArray(promptLibrary.libraries) ? promptLibrary.libraries.filter(Boolean) : [];
    if(!libs.length) return [{id:'system', name:L('系统提示词库','System prompt library'), system:true, items:[], categories:[]}];
...
function promptCategoryLabel(category='custom'){
    ...
    const map = {view:L('视角','View'), storyboard:L('分镜','Storyboard'), character:L('角色','Character'), product:L('产品','Product'), lighting:L('光影','Lighting'), mine:L('我的','Mine'), custom:L('我的','Mine')};
    return map[category] || category || L('自定义','Custom');
}
...
function mediaAssetItems(items){
    return (Array.isArray(items) ? items : []).filter(item =>
        [item?.name, item?.url].some(value => CANVAS_MEDIA_EXTS.test(String(value || '')))
        || /^(image|video|audio)\//i.test(String(item?.mime || item?.mimeType || ''))
        || /^data:(image|video|audio)\//i.test(String(item?.url || ''))
        || ([item?.name, item?.url].every(value => !/(?:^|\/)[^/?#]+\.[a-z0-9]{1,10}(?:[?#]|$)/i.test(String(value || ''))) && ['image','video','audio'].includes(String(item?.kind || item?.type || '').toLowerCase()))
    );
}
function assetClassificationChips(item, limit=10){
    ...
    return flat.slice(0, limit).map(entry => {
        const label = entry?.label || entry?.dimension || L('分类','Category');
        const tag = entry?.tag || '';
...
function assetClassificationEntriesForLibrary(lib=activeAssetLibrary()){
    const items = [];
    (lib?.categories || []).filter(cat => (cat.type || 'image') === 'image').forEach(cat => items.push(...mediaAssetItems(cat.items)));
    ...
    (items || []).forEach(item => {
        ...
        flat.forEach(entry => {
            ...
            const current = groups.get(key) || {
                ...
                label:String(entry.label || entry.dimension || L('分类','Category')),
...
function assetItemsForClassFilter(key=activeAssetClassFilter){
    ...
    if(!parsed) return [];
    return mediaAssetItems(assetCategories().flatMap(cat => cat.items || [])).filter(item => {
        const flat = Array.isArray(item?.classification?.flat) ? item.classification.flat : [];
...
function workflowKindLabel(item){
    ...
    const url = String(item?.url || '').toLowerCase();
    if(format === 'json' || url.endsWith('.json')) return L('JSON 工作流','JSON workflow');
    return L('ZIP 工作流包','ZIP workflow package');
}
function assetKindLabel(item){
    const kind = assetKind(item);
    if(kind === 'video') return L('视频','Video');
    if(kind === 'audio') return L('音频','Audio');
    if(kind === 'text') return L('文本','Text');
    return L('图片','Image');
}
...
function assetPreviewUrl(url, w=256){
    ...
    if(!(path.startsWith('/canvas-output/') || path.startsWith('/canvas-assets/'))) return raw;
...
function assetThumb(item){
    ...
    if(kind === 'audio') return `<div class="asset-file-icon"><i data-lucide="file-audio"></i><span>${L('音频','Audio')}</span></div>`;
    if(kind === 'text') return `<div class="asset-file-icon"><i data-lucide="file-text"></i><span>${L('文本','Text')}</span></div>`;
...
function renderLocalCaptionTools(imageCount){
    ...
    return `
        <div class="local-caption-tools">
            ...
            <select id="localCaptionProvider" class="manage-select" title="${L('平台','Platform')}" ${providers.length ? '' : 'disabled'}>
                ${providers.length ? providers.map(p => `<option value="${escapeAttr(p.id)}" ${p.id === localCaptionProvider ? 'selected' : ''}>${escapeHtml(p.name || p.id)}</option>`).join('') : '<option value="">暂无聊天平台</option>'}
            </select>
            <select id="localCaptionModel" class="manage-select" title="${L('模型','Model')}" ${models.length ? '' : 'disabled'}>
                ${models.length ? models.map(m => `<option value="${escapeAttr(m)}" ${m === localCaptionModel ? 'selected' : ''}>${escapeHtml(m)}</option>`).join('') : '<option value="">暂无模型</option>'}
            </select>
            <input id="localCaptionPrompt" class="local-caption-prompt-input" type="text" value="${escapeAttr(localCaptionPrompt || L('描述图片','Describe the image'))}" placeholder="${L('描述图片','Describe the image')}">
            <button class="asset-btn" type="button" data-local-classify-run ${classifyDisabled ? 'disabled' : ''}><i data-lucide="${localClassifyBusy ? 'loader-2' : 'tags'}"></i><span>${localClassifyBusy ? L('分类中','Classifying') : L('智能分类','Smart classify')}</span></button>
            <button class="asset-btn primary" type="button" data-local-caption-run ${captionDisabled ? 'disabled' : ''}><i data-lucide="${localCaptionBusy ? 'loader-2' : 'wand-sparkles'}"></i><span>${localCaptionBusy ? L('反推中','Reversing') : L('提示词反推','Reverse caption')}</span></button>
            <button class="asset-icon-btn" type="button" data-local-classify-toggle title="${L('智能分类要求','Smart classification requirements')}"><i data-lucide="${localClassifyPromptOpen ? 'chevron-up' : 'sliders-horizontal'}"></i></button>
            </div>
            ${localClassifyPromptOpen ? `<textarea id="localClassifyPrompt" class="local-classify-prompt-input" placeholder="${L('默认已内置室内/室外、空间、主体、光影、模特、风格、色彩等分类。这里可以追加你的分类要求，例如：增加家装风格、镜头焦段、商业用途、人物年龄段、产品材质。','Built-in categories include indoor/outdoor, space, subject, lighting, model, style, color. Append extra requirements, e.g. interior style, focal length, commercial use, age group, product material.')}">${escapeHtml(localClassifyPrompt || '')}</textarea>` : ''}
        </div>
    `;
}
...
async function loadLocalAssets(){
    try {
        ...
        localUploadTree = data.tree || {id:'__root__', path:'', name:L('全部上传','All uploads'), count:localAssets.length, items:[], children:[]};
    } catch(err) {
        localAssets = [];
        localUploadTree = {id:'__root__', path:'', name:L('全部上传','All uploads'), count:0, items:[], children:[]};
    }
...
async function registerSharedFolder(){
    const tip = L('请输入要登记的共享文件夹路径（必须位于项目目录内，例如 assets\\library 或 output）：','Enter the shared folder path to register (must be inside the project, e.g. assets\\library or output):');
    const path = window.prompt(tip, '');
    if(!String(path || '').trim()) return;
    try {
        setStatus(L('正在登记共享文件夹...','Registering shared folder...'));
        const data = await apiJson('/api/shared-folders', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
        ...
        setStatus(L(`已登记「${folder?.name || L('共享文件夹','Shared folder')}」`,`Registered "${folder?.name || 'Shared folder'}"`));
    } catch(err) {
        setStatus(err.message || L('登记共享文件夹失败','Register shared folder failed'));
    }
}
async function unregisterSharedFolder(folderId){
    ...
    try {
        ...
        await loadSharedFolders();
        render();
        setStatus(L('已移除共享文件夹登记（不会删除磁盘文件）','Removed shared folder registration (disk files kept)'));
    } catch(err) {
        setStatus(err.message || L('移除共享文件夹失败','Remove shared folder failed'));
    }
}
...
async function openSharedFolder(folderId){
    if(!folderId) return;
    try {
        setStatus(L('正在读取共享文件夹...','Reading shared folder...'));
        ...
        activeSharedFolderName = data.folder?.name || tree?.name || L('共享文件夹','Shared folder');
        ...
        normalizeLocalState();
        render();
        setStatus(L(`已读取「${activeSharedFolderName}」`,`Read "${activeSharedFolderName}"`));
    } catch(err) {
        setStatus(err.message || L('读取共享文件夹失败','Read shared folder failed'));
    }
}
function currentAssetItems(){
    ...
    return mediaAssetItems(source).filter(item => {
        if(!query) return true;
...
function canvasAssetCategories(){
    const cats = Array.isArray(canvasAssetsData.categories) && canvasAssetsData.categories.length
        ...
        : [
            {id:'smart', name:L('智能画布','Smart canvas'), count:(canvasAssetsData.items || []).filter(item => item.canvas_kind === 'smart').length, canvas_count:(canvasAssetsData.canvases || []).filter(item => item.kind === 'smart').length},
            {id:'classic', name:L('普通画布','Classic canvas'), count:(canvasAssetsData.items || []).filter(item => item.canvas_kind !== 'smart').length, canvas_count:(canvasAssetsData.canvases || []).filter(item => item.kind !== 'smart').length}
        ];
    return cats.map(cat => ({
        ...cat,
        count:mediaAssetItems(uniqueCanvasAssets(canvasAssetsData.items || [])).filter(item => (item.canvas_kind || 'classic') === cat.id).length
    }));
}
function activeCanvasAssetCategoryInfo(){
    return canvasAssetCategories().find(cat => cat.id === activeCanvasAssetCategory) || canvasAssetCategories()[0] || {id:'smart', name:L('智能画布','Smart canvas'), count:0, canvas_count:0};
}
...
function canvasAssetCountForCanvas(canvasId){
    return mediaAssetItems(uniqueCanvasAssets(canvasAssetsData.items || [])).filter(item => item.canvas_id === canvasId).length;
}
...
function canvasAssetOpenUrl(canvas){
    ...
    return canvas.kind === 'smart' ? `/smart-canvas-core/smart-canvas.html?id=${id}&project=${project}&system=canvas` : `/index.html?system=canvas&id=${id}&project=${project}`;
}
...
function canvasAssetViewTitle(){
    return activeCanvasAssetCanvas()?.title || activeCanvasAssetCategoryInfo().name || L('画布资产','Canvas asset');
}
function canvasAssetViewSubtitle(items){
    const canvas = activeCanvasAssetCanvas();
    if(canvas) return L(`${canvasKindLabel(canvas.kind)} / ${items.length} 个资产 / ${escapeHtml(canvasAssetSortLabel())}`,`${canvasKindLabel(canvas.kind)} / ${items.length} assets / ${escapeHtml(canvasAssetSortLabel())}`);
    const activeCat = activeCanvasAssetCategoryInfo();
    return L(`${Number(activeCat.canvas_count || 0)} 个画布 / ${items.length} 个资产 / ${escapeHtml(canvasAssetSortLabel())}`,`${Number(activeCat.canvas_count || 0)} canvases / ${items.length} assets / ${escapeHtml(canvasAssetSortLabel())}`);
}
function canvasAssetKindLabel(item){
    const kind = assetKind(item);
    if(kind === 'video') return L('视频','Video');
    if(kind === 'audio') return L('音频','Audio');
    if(String(item?.kind || '').toLowerCase() === 'text') return L('文本','Text');
    return L('图片','Image');
}
function canvasKindLabel(kind){
    return kind === 'smart' ? L('智能画布','Smart canvas') : L('普通画布','Classic canvas');
}
function canvasAssetSortLabel(){
    const map = {canvas_asc:L('画布名称','Canvas name'), updated_desc:L('最近更新','Recently updated'), updated_asc:L('最早更新','Oldest updated'), name_asc:L('名称 A-Z','Name A-Z'), kind:L('类型','Type')};
    return map[canvasAssetSort] || map.canvas_asc;
}
function unifiedCanvasAssetItems(){
    const q = String(canvasAssetQuery || '').trim().toLowerCase();
    return (canvasAssetCenterItems || []).filter(item => {
        if(canvasAssetShowTrash ? !item.deleted_at : item.deleted_at) return false;
        if(canvasAssetScope === 'favorite' && !item.favorite) return false;
        if(canvasAssetScope === 'project' && !(item.project_id || item.source_canvas_id)) return false;
        if(canvasAssetTypeFilter !== 'all' && assetKind(item) !== canvasAssetTypeFilter) return false;
        if(canvasAssetProjectFilter !== 'all' && String(item.project_id || '') !== canvasAssetProjectFilter) return false;
        if(!q) return true;
        return [item.name, item.prompt, item.model, item.project_name, item.source_canvas_id, item.source_node_id, canvasAssetKindLabel(item)].join(' ').toLowerCase().includes(q);
    });
}
function currentCanvasAssetItems(){
    const unified = unifiedCanvasAssetItems();
    let list = unified.length || canvasAssetCenterItems.length ? unified : uniqueCanvasAssets(canvasAssetsData.items || []).filter(item => {
        ...
        const q = String(canvasAssetQuery || '').trim().toLowerCase();
        return !q || [item.name, item.url, item.canvas_title, item.node_title, item.node_type, canvasAssetKindLabel(item), canvasKindLabel(item.canvas_kind)].join(' ').toLowerCase().includes(q);
    });
    list = list.slice();
    const byTime = item => Number(item.created_at || item.canvas_updated_at || 0);
    const byCanvasName = (a, b) => String(a.source_canvas_id || a.canvas_title || '').localeCompare(String(b.source_canvas_id || b.canvas_title || ''), 'zh-Hans-CN', {numeric:true, sensitivity:'base'}) || byTime(b) - byTime(a);
...
function groupCanvasAssetItems(items){
    ...
    (items || []).forEach(item => {
        ...
        if(!map.has(key)){
            const group = {
                key,
                title:item.canvas_title || L('未命名画布','Untitled canvas'),
...
function assetMoveTargets(){
    ...
    assetLibraries().forEach(lib => {
        (lib.categories || []).filter(cat => (cat.type || 'image') === 'image').forEach(cat => {
            ...
            if(key !== currentKey) targets.push({key, libraryId:lib.id, categoryId:cat.id, label:`${lib.name || L('资产库','Asset library')} / ${cat.name || L('分组','Group')}`});
        });
    });
    return targets;
}
...
const AVATAR_PLATFORM_LABELS = {apimart:'APIMart', volcengine:L('火山引擎','Volcengine')};
...
function avatarPlatformLabel(platform){
    return AVATAR_PLATFORM_LABELS[String(platform || '')] || String(platform || L('平台','Platform'));
}
...
function avatarProviderOptionLabel(p){
    ...
    if(!platform) return `${name}（${L('暂不支持，待接入','Not yet supported')}）`;
    if(!providerAvatarSupported(p)) return `${name}（${avatarPlatformLabel(platform)}·${L('待接入','pending')}）`;
...
function findCanvasAssetItem(id){
    return (canvasAssetCenterItems || []).find(item => item.id === id) || (canvasAssetsData.items || []).find(item => item.id === id) || null;
}
...
async function refreshCanvasAssets(){
    try {
        setStatus(L('正在刷新画布资产...','Refreshing canvas assets...'));
        const [data, center] = await Promise.all([
            apiJson('/api/canvas-assets'),
            apiJson(`/api/canvas/asset-center?includeDeleted=${canvasAssetShowTrash ? 'true' : 'false'}`)
        ]);
        canvasAssetsData = {
        ...
        canvasAssetCenterItems = Array.isArray(center.items) ? center.items : [];
        canvasAssetCenterProjects = Array.isArray(center.projects) ? center.projects : [];
        normalizeCanvasAssetState();
        render();
        setStatus(L('画布资产已刷新','Canvas assets refreshed'));
    } catch(err) {
        setStatus(err.message || L('刷新画布资产失败','Refresh canvas assets failed'));
    }
}
async function loadAll(){
    setStatus(L('加载中...','Loading...'));
    const [assetData, promptData, providerData, canvasAssetData, assetCenterData] = await Promise.all([
        ...
        apiJson('/api/canvas/asset-center?includeDeleted=true').catch(() => ({items:[], projects:[]})),
        loadSharedFolders(),
        loadLocalAssets()
    ]);
    ...
    canvasAssetCenterItems = Array.isArray(assetCenterData.items) ? assetCenterData.items : [];
    canvasAssetCenterProjects = Array.isArray(assetCenterData.projects) ? assetCenterData.projects : [];
    // 刷新时默认回到「默认资产库」
    const libs = assetLibraries();
...
function renderCanvasAssetsManager(){
    ...
    const total = mediaAssetItems(uniqueCanvasAssets(canvasAssetsData.items || [])).length;
    const detail = selectedCanvasAsset();
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head"><div class="panel-title"><strong>${L('画布分类','Canvas categories')}</strong><span>${L('从所有画布拉取资产','Pull assets from all canvases')}</span></div></div>
            <div class="nav-scroll">
                <div class="nav-tree canvas-asset-tree">
                    ${canvasAssetCategories().map(cat => renderCanvasAssetTreeBranch(cat)).join('')}
                </div>
                <div class="nav-hint">${L(`共 ${canvasAssetsData.canvases?.length || 0} 个画布，${total} 个可下载资产。`,`${canvasAssetsData.canvases?.length || 0} canvases, ${total} downloadable assets.`)}</div>
            </div>
        </aside>
        <section class="asset-panel asset-content ${canvasAssetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <button class="asset-btn ${canvasAssetScope === 'all' && !canvasAssetShowTrash ? 'primary' : ''}" type="button" data-canvas-asset-scope="all"><i data-lucide="layout-dashboard"></i><span>${L('全部','All')}</span></button>
                    <button class="asset-btn ${canvasAssetScope === 'favorite' && !canvasAssetShowTrash ? 'primary' : ''}" type="button" data-canvas-asset-scope="favorite"><i data-lucide="star"></i><span>${L('收藏','Favorites')}</span></button>
                    <button class="asset-btn ${canvasAssetScope === 'project' && !canvasAssetShowTrash ? 'primary' : ''}" type="button" data-canvas-asset-scope="project"><i data-lucide="folder-kanban"></i><span>${L('项目','Projects')}</span></button>
                    <button class="asset-btn ${canvasAssetShowTrash ? 'primary' : ''}" type="button" data-canvas-asset-trash><i data-lucide="trash-2"></i><span>${L('回收站','Trash')}</span></button>
                    <select id="canvasAssetTypeFilter" class="manage-select" title="${L('媒体类型','Media type')}">
                        <option value="all" ${canvasAssetTypeFilter === 'all' ? 'selected' : ''}>${L('全部类型','All types')}</option>
                        <option value="image" ${canvasAssetTypeFilter === 'image' ? 'selected' : ''}>${L('图片','Images')}</option>
                        <option value="video" ${canvasAssetTypeFilter === 'video' ? 'selected' : ''}>${L('视频','Videos')}</option>
                    </select>
                    <select id="canvasAssetProjectFilter" class="manage-select" title="${L('项目','Project')}">
                        <option value="all">${L('全部项目','All projects')}</option>
                        ${canvasAssetCenterProjects.map(project => `<option value="${escapeAttr(project.id)}" ${canvasAssetProjectFilter === project.id ? 'selected' : ''}>${escapeHtml(project.name || project.id)}</option>`).join('')}
                    </select>
                    <button class="asset-btn" type="button" data-canvas-asset-refresh title="重新读取画布中的图片、视频、音频资源"><i data-lucide="refresh-cw"></i><span>${L('刷新资源','Refresh resources')}</span></button>
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="canvasAssetSearch" class="asset-search" type="search" value="${escapeAttr(canvasAssetQuery)}" placeholder="${L('搜索画布资产','Search canvas assets')}"></label>
                    <select id="canvasAssetSort" class="manage-select canvas-sort-select" title="${L('排序方法','Sort method')}">
                        <option value="canvas_asc" ${canvasAssetSort === 'canvas_asc' ? 'selected' : ''}>${L('画布名称','Canvas name')}</option>
                        <option value="updated_desc" ${canvasAssetSort === 'updated_desc' ? 'selected' : ''}>${L('最近更新','Recently updated')}</option>
                        <option value="updated_asc" ${canvasAssetSort === 'updated_asc' ? 'selected' : ''}>${L('最早更新','Oldest updated')}</option>
                        <option value="name_asc" ${canvasAssetSort === 'name_asc' ? 'selected' : ''}>${L('资产名称','Asset name')}</option>
                        <option value="kind" ${canvasAssetSort === 'kind' ? 'selected' : ''}>${L('类型','Type')}</option>
                    </select>
                    <button class="asset-btn ${canvasAssetManageMode ? 'primary' : ''}" type="button" data-canvas-asset-manage ${total ? '' : 'disabled'}><i data-lucide="list-checks"></i><span>${canvasAssetManageMode ? L('完成管理','Done') : L('批量管理','Batch manage')}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span>${L(`已选择 ${selectedCanvasAssetIds.size} 个画布资产，支持拖拽框选或逐个勾选。`,`Selected ${selectedCanvasAssetIds.size} canvas assets. Drag-select or tick individually.`)}</span>
                <div class="asset-tools">
                    <button class="asset-btn" type="button" data-canvas-asset-select-all ${items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                    <button class="asset-btn" type="button" data-canvas-asset-clear-selection ${selectedCanvasAssetIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>${L('清空','Clear')}</span></button>
                    <button class="asset-btn primary" type="button" data-canvas-asset-download-selected ${selectedCanvasAssetIds.size ? '' : 'disabled'}><i data-lucide="download"></i><span>${L('下载所选','Download selected')}</span></button>
                </div>
            </div>
            <div class="content-scroll">
                <div class="asset-grid">
                    ${groups.map(group => renderCanvasAssetGroup(group)).join('')}
                    ${items.length ? '' : `<div class="empty-state">${L('当前分类没有可下载的画布资产。可以点击"刷新资源"，或在画布中生成/导入图片、视频、音频后再查看。','No downloadable canvas assets in this category. Click "Refresh resources", or generate/import images, videos or audio in a canvas.')}</div>`}
                </div>
            </div>
        </section>
...
function renderCanvasAssetTreeBranch(cat){
    ...
    return `<div class="tree-branch ${cat.id === activeCanvasAssetCategory ? 'expanded' : ''}">
        <button class="tree-row tree-parent ${activeParent ? 'active' : ''} ${containsActive ? 'contains-active' : ''}" type="button" data-canvas-asset-cat="${escapeAttr(cat.id)}">
            ...
            <span class="tree-row-name">${escapeHtml(cat.name || L('画布','Canvas'))}</span>
        ...
        <div class="tree-children">
            ...
            }).join('') : `<div class="tree-empty">${L('暂无画布','No canvases')}</div>`}
        </div>
    </div>`;
}
function renderCanvasAssetGroup(group){
    ...
    return `<section class="canvas-asset-group">
        <div class="canvas-asset-group-head">
            ...
            <small>${L(`${(group.items || []).length} 个资产`,`${(group.items || []).length} assets`)}</small>
        </div>
...
function renderCanvasAssetCard(item){
    return `<article class="asset-card canvas-asset-card ${item.id === selectedCanvasAssetId ? 'active' : ''}" data-canvas-asset-card="${escapeAttr(item.id)}">
        ...
        <div class="asset-card-body">
            ...
            <div class="asset-card-meta">${escapeHtml(canvasAssetKindLabel(item))} · ${escapeHtml(item.project_name || item.canvas_title || L('未归类','Unassigned'))}</div>
            <div class="asset-card-actions">
                <button type="button" data-canvas-asset-favorite="${escapeAttr(item.id)}" title="${item.favorite ? L('取消收藏','Unfavorite') : L('收藏','Favorite')}"><i data-lucide="${item.favorite ? 'star-off' : 'star'}"></i></button>
                ${item.deleted_at ? `<button type="button" data-canvas-asset-restore="${escapeAttr(item.id)}" title="${L('恢复','Restore')}"><i data-lucide="rotate-ccw"></i></button>` : `<button type="button" data-canvas-asset-delete="${escapeAttr(item.id)}" title="${L('移入回收站','Move to trash')}"><i data-lucide="trash-2"></i></button>`}
            </div>
        </div>
    </article>`;
}
...
function renderCanvasAssetDetail(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>${L('画布资产详情','Canvas asset detail')}</strong><span>${L('选择一个画布资产查看详情','Select a canvas asset to view detail')}</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="layout-dashboard"></i><span>${L('暂无画布资产','No canvas assets')}</span></div></div>`;
    const kind = assetKind(item);
    const canPreview = ['image','video'].includes(kind);
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>${L('画布资产详情','Canvas asset detail')}</strong><span>${escapeHtml(canvasAssetKindLabel(item))}</span></div>
            <div class="panel-actions">
                ...
                <button class="asset-icon-btn" type="button" data-canvas-asset-favorite="${escapeAttr(item.id)}" title="${item.favorite ? L('取消收藏','Unfavorite') : L('收藏','Favorite')}"><i data-lucide="${item.favorite ? 'star-off' : 'star'}"></i></button>
                ${item.deleted_at ? `<button class="asset-icon-btn" type="button" data-canvas-asset-restore="${escapeAttr(item.id)}" title="${L('恢复','Restore')}"><i data-lucide="rotate-ccw"></i></button>` : `<button class="asset-icon-btn danger" type="button" data-canvas-asset-delete="${escapeAttr(item.id)}" title="${L('移入回收站','Move to trash')}"><i data-lucide="trash-2"></i></button>`}
                ...
                <button class="asset-btn primary" type="button" data-canvas-asset-download="${escapeAttr(item.id)}"><i data-lucide="download"></i><span>${L('下载','Download')}</span></button>
            </div>
        </div>
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                <div class="detail-name">${escapeHtml(item.name || L('画布资产','Canvas asset'))}</div>
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(canvasAssetKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>${L('画布分类','Canvas category')}</span><strong>${escapeHtml(canvasKindLabel(item.canvas_kind))}</strong></div>
                    ...
                    <div class="detail-meta"><span>${L('更新时间','Updated')}</span><strong>${escapeHtml(formatDate(item.canvas_updated_at || item.created_at))}</strong></div>
                    ...
                    <div class="detail-meta"><span>${L('节点类型','Node type')}</span><strong>${escapeHtml(item.node_type || '-')}</strong></div>
                </div>
...
function renderHeadTreeInlineEdit(edit, inputId, saveAttr, cancelAttr){
    ...
    const label = edit.label || L('名称','Name');
    return `<div class="head-inline-edit">
        ...
        <button type="button" class="primary" ${saveAttr} title="${L('保存','Save')}"><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
        <button type="button" ${cancelAttr} title="${L('取消','Cancel')}"><i data-lucide="x"></i><span>${L('取消','Cancel')}</span></button>
    </div>`;
}
...
function localUploadFolderTitle(){
    ...
    if(entry) return entry.tag || L('智能分类','Smart classify');
    if(!activeLocalUploadFolder) return L('本地上传','Local uploads');
    return localUploadFolderByPath(activeLocalUploadFolder)?.name || activeLocalUploadFolder.split('/').pop() || L('本地上传','Local uploads');
}
function localUploadViewSubtitle(items){
    const entry = activeLocalUploadClassEntry();
    if(entry) return L(`智能分类：${entry.label} / ${items.length} 个素材`,`Smart: ${entry.label} / ${items.length} assets`);
    return L(`${items.length} / ${(localAssets || []).length} 个素材`,`${items.length} / ${(localAssets || []).length} assets`);
}
...
function renderLocalUploadFolderBranch(folder, depth=0){
    ...
    return `<div class="tree-branch">
        <button class="tree-row ${depth ? 'tree-child' : 'tree-parent'} ${active ? 'active' : ''} ${contains ? 'contains-active' : ''}" type="button" data-localup-folder="${escapeAttr(path)}">
            ...
            <span class="tree-row-name">${escapeHtml(folder.name || L('文件夹','Folder'))}</span>
...
function renderLocalUploadSmartClassTree(){
    ...
    return `<div class="tree-smart-class">
        <button class="tree-row tree-parent ${activeLocalUploadClassFilter ? 'contains-active' : ''}" type="button" data-localup-class-root>
            ...
            <span class="tree-row-name">${L('智能分类','Smart classify')}</span>
        ...
        </div>`).join('') : `<div class="tree-empty">${L('暂无分类，先选择图片点"智能分类"','No categories yet. Select images and click "Smart Classify"')}</div>`}
    </div>`;
}
...
function renderLocalManager(){
    ...
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>${L('本地上传','Local uploads')}</strong><span>${L('批量上传到 assets/uploads','Batch upload to assets/uploads')}</span></div>
                <div class="panel-actions compact-actions">
                    <button class="asset-icon-btn" type="button" data-localup-folder-new title="${L('新建文件夹','New folder')}"><i data-lucide="folder-plus"></i></button>
                    <button class="asset-icon-btn" type="button" data-localup-folder-rename title="${L('重命名文件夹','Rename folder')}" ${activeLocalUploadFolder ? '' : 'disabled'}><i data-lucide="pencil"></i></button>
                </div>
            </div>
            <div class="nav-scroll">
                <div class="nav-tree">
                    ${renderLocalUploadFolderBranch(localUploadTree || {path:'', name:L('全部上传','All uploads'), count:total, children:[]})}
                    ${renderLocalUploadSmartClassTree()}
                </div>
                <div class="nav-hint" style="padding:10px 12px;font-size:12px;opacity:.7;">${L('选择图片/视频/音频文件即可上传，文件保存在项目 assets/uploads 目录。','Select image/video/audio files to upload; files are saved under assets/uploads.')}</div>
            </div>
        </aside>
        <section class="asset-panel asset-content ${localUploadManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="localUploadSearch" class="asset-search" type="search" value="${escapeAttr(localUploadQuery)}" placeholder="${L('搜索本地上传','Search local uploads')}"></label>
                    <button class="asset-btn primary" type="button" data-localup-upload><i data-lucide="upload"></i><span>${L('上传文件','Upload file')}</span></button>
                    <button class="asset-btn ${localUploadManageMode ? 'primary' : ''}" type="button" data-localup-manage ${total ? '' : 'disabled'}><i data-lucide="list-checks"></i><span>${localUploadManageMode ? L('完成管理','Done') : L('批量管理','Batch manage')}</span></button>
                </div>
            </div>
            <div class="manage-tools local-manage-tools">
                <div class="manage-group manage-select-group">
                    <span class="manage-group-title">${L('管理','Manage')}</span>
                    <span class="manage-summary">${L(`已选 ${selectedLocalUploadIds.size} 个，其中 ${imageCount} 张图片`,`Selected ${selectedLocalUploadIds.size}, ${imageCount} images`)}</span>
                    <div class="asset-tools local-manage-actions">
                        <button class="asset-btn" type="button" data-localup-select-all ${items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                        <button class="asset-btn" type="button" data-localup-clear ${selectedLocalUploadIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>${L('清空','Clear')}</span></button>
                        <button class="asset-btn" type="button" data-localup-download-selected ${selectedLocalUploadIds.size ? '' : 'disabled'}><i data-lucide="download"></i><span>${L('下载','Download')}</span></button>
                        <button class="asset-btn" type="button" data-localup-canvas-selected ${selectedLocalUploadIds.size ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>${L('复制到画布','Copy to canvas')}</span></button>
                        <button class="asset-btn" type="button" data-localup-cut-selected ${selectedLocalUploadIds.size ? '' : 'disabled'}><i data-lucide="scissors"></i><span>${L('剪切/移动','Cut / Move')}</span></button>
                        <button class="asset-btn danger" type="button" data-localup-delete-selected ${selectedLocalUploadIds.size ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${L('删除','Delete')}</span></button>
                    </div>
                </div>
                <div class="manage-group manage-ai-group">
                    <span class="manage-group-title">${L('AI 处理','AI processing')}</span>
...
function renderLocalUploadAddCard(){
    return `<button id="localUploadDrop" class="upload-grid-card" type="button" data-localup-upload>
        ...
        <span class="upload-body">
            <strong>${L('上传本地素材','Upload local asset')}</strong>
            <small>${L('拖入文件或点击上传','Drag file or click to upload')}</small>
        </span>
    </button>`;
}
function renderLocalUploadCard(item){
    ...
    return `<article class="asset-card ${item.id === selectedLocalUploadId ? 'active' : ''}" data-localup-card="${escapeAttr(item.id)}">
        ...
        <div class="asset-card-body">
            <div class="asset-card-name" data-localup-rename="${escapeAttr(item.id)}" title="${escapeAttr(item.name || '')}">${escapeHtml(item.name || L('本地素材','Local asset'))}</div>
            <div class="asset-card-meta">${escapeHtml(assetKindLabel(item))} · ${escapeHtml(formatFileSize(item.size))}${hasCaption ? L(' · 有提示词',' · has caption') : ''}</div>
        </div>
    </article>`;
}
function renderLocalUploadDetail(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>${L('素材预览','Asset preview')}</strong><span>${L('选择一个素材查看详情','Select an asset to view detail')}</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="image"></i><span>${L('暂无可预览素材','No previewable asset')}</span></div></div>`;
    const isImage = assetKind(item) === 'image';
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>${L('素材预览','Asset preview')}</strong><span>${escapeHtml(assetKindLabel(item))}</span></div>
            <div class="panel-actions">
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                <input class="detail-name-input" data-localup-inline-name="${escapeAttr(item.id)}" type="text" value="${escapeAttr(item.name || L('本地素材','Local asset'))}" title="${L('直接修改名称','Edit name directly')}">
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(assetKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>${L('大小','Size')}</span><strong>${escapeHtml(formatFileSize(item.size))}</strong></div>
                    <div class="detail-meta"><span>${L('上传时间','Uploaded')}</span><strong>${escapeHtml(formatDate((item.created_at||0)*1000))}</strong></div>
                    <div class="detail-meta"><span>${L('来源','Source')}</span><strong>本地上传</strong></div>
                </div>
                <div class="detail-url">${escapeHtml(item.url || '')}</div>
                ${isImage ? `<div class="detail-caption-card">
                    <div class="detail-caption-head"><strong>${L('智能分类','Smart classify')}</strong></div>
                    <div class="detail-classification-body">${renderClassificationChips(item, 28, {kind:'localup'}) || `<span class="classification-empty">${L('暂无智能分类，可以上传时自动生成或后续批量分类。','No smart classification yet. Generate on upload or batch classify later.')}</span>`}</div>
                </div>` : ''}
                ${isImage ? `
                    <div class="detail-caption-card">
                        <div class="detail-caption-head">
                            <strong>${L('反推提示词','Caption reverse')}</strong>
                            <div class="detail-caption-actions">
                                ...
                                <button class="asset-btn" type="button" data-localup-caption-copy="${escapeAttr(item.id)}"><i data-lucide="copy"></i><span>${L('复制','Copy')}</span></button>
                                <button class="asset-btn primary" type="button" data-localup-caption-save="${escapeAttr(item.id)}"><i data-lucide="save"></i><span>${L('保存','Save')}</span></button>
                            </div>
                        </div>
                        <textarea id="localUploadCaptionEdit" class="detail-caption-textarea" placeholder="${L('暂无反推提示词，可以批量反推后自动写入，也可以在这里手动编辑。','No caption yet. Batch reverse or edit manually here.')}">${escapeHtml(item.caption || '')}</textarea>
                    </div>
                ` : ''}
            </div>
        </div>
...
function renderLocalClipboardBar(){
    ...
    const modeLabel = localClipboard.mode === 'cut' ? L('剪切','Cut') : L('复制','Copy');
    const target = activeAssetCategory();
    return `<div class="asset-clipboard-bar">
        <div class="asset-clipboard-info"><i data-lucide="clipboard"></i><span>${L(`${escapeHtml(modeLabel)}了 ${localClipboard.items.length} 个本地素材，目标：${escapeHtml(activeAssetLibrary()?.name || L('图片资产','Image assets'))} / ${escapeHtml(target?.name || L('未选择分组','No group selected'))}`,`${escapeHtml(modeLabel)} ${localClipboard.items.length} local assets. Target: ${escapeHtml(activeAssetLibrary()?.name || 'Image assets')} / ${escapeHtml(target?.name || 'No group selected')}`)}</span></div>
        <div class="asset-tools">
            <button class="asset-btn primary" type="button" data-local-import-clipboard ${target ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>${L('导入到图片资产','Import to image assets')}</span></button>
            <button class="asset-icon-btn" type="button" data-local-clear-clipboard title="${L('清空本地剪贴板','Clear local clipboard')}"><i data-lucide="x"></i></button>
        </div>
    </div>`;
}
function renderLocalDetail(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>${L('本地预览','Local preview')}</strong><span>${L('选择一个本地素材查看详情','Select a local asset to view detail')}</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="folder-open"></i><span>${L('暂无可预览素材','No previewable asset')}</span></div></div>`;
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>${L('本地预览','Local preview')}</strong><span>${escapeHtml(assetKindLabel(item))}</span></div>
            <div class="panel-actions">
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                <div class="detail-name">${escapeHtml(item.name || L('本地素材','Local asset'))}</div>
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(assetKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>${L('大小','Size')}</span><strong>${escapeHtml(formatFileSize(item.size))}</strong></div>
                    <div class="detail-meta"><span>${L('修改时间','Modified')}</span><strong>${escapeHtml(formatDate(item.lastModified))}</strong></div>
                    <div class="detail-meta"><span>${L('来源','Source')}</span><strong>${escapeHtml(activeSharedFolderName || L('共享文件夹','Shared folder'))}</strong></div>
                </div>
...
function renderAssetManager(){
    ...
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>${L('资产层级','Asset hierarchy')}</strong><span>${L('先选库，再选分组','Select library first, then group')}</span></div>
                <div class="panel-actions compact-actions">
                    ${assetTreeEdit?.placement === 'head' ? '' : `<button class="asset-icon-btn" type="button" data-asset-lib-new title="${L('新建资产库','New asset library')}"><i data-lucide="plus"></i></button>`}
        ...
        <section class="asset-panel asset-content ${assetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="assetSearch" class="asset-search" type="search" value="${escapeAttr(assetQuery)}" placeholder="${L('搜索素材','Search assets')}"></label>
                    <button class="asset-btn ${assetManageMode ? 'primary' : ''}" type="button" data-asset-manage><i data-lucide="list-checks"></i><span>${assetManageMode ? L('完成管理','Done') : L('批量管理','Batch manage')}</span></button>
                </div>
            </div>
            ...
            <div class="manage-tools">
                <div class="manage-group manage-select-group">
                    <span class="manage-group-title">${L('管理','Manage')}</span>
                    <span class="manage-summary">${L(`已选 ${selectedAssetIds.size} 个，其中 ${imageCount} 张图片`,`Selected ${selectedAssetIds.size}, ${imageCount} images`)}</span>
                    <div class="asset-tools">
                        <button class="asset-btn" type="button" data-asset-select-all ${items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                        <button class="asset-btn" type="button" data-asset-clear-selection ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>${L('清空','Clear')}</span></button>
                        <button class="asset-btn" type="button" data-asset-cut-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="scissors"></i><span>${L('剪切','Cut')}</span></button>
                        <button class="asset-btn" type="button" data-asset-copy-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="copy"></i><span>${L('复制','Copy')}</span></button>
                        <button class="asset-btn" type="button" data-asset-download-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="download"></i><span>${L('下载所选','Download selected')}</span></button>
                        <button class="asset-btn" type="button" data-asset-copy-to-canvas ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>${L('复制到画布','Copy to canvas')}</span></button>
                        <button class="asset-btn danger" type="button" data-asset-delete-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${L('删除所选','Delete selected')}</span></button>
                    </div>
                </div>
                <div class="manage-group manage-ai-group">
                    <span class="manage-group-title">${L('AI 处理','AI processing')}</span>
                    <button class="asset-btn" type="button" data-asset-classify-selected ${imageCount && !assetClassifyBusy ? '' : 'disabled'}><i data-lucide="${assetClassifyBusy ? 'loader-2' : 'tags'}"></i><span>${assetClassifyBusy ? L('分类中','Classifying') : L('智能分类','Smart classify')}</span></button>
                </div>
            </div>
            <div class="content-scroll">
                <div class="asset-grid">
                    ...
                    ${items.length ? '' : `<div class="empty-state">${L('当前分组还没有素材，可以上传，或从智能画布输出保存到素材库。','No assets in this group yet. Upload, or save from Smart Canvas output.')}</div>`}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">
...
function renderWorkflowManager(){
    ...
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>${L('工作流层级','Workflow hierarchy')}</strong><span>${L('独立管理工作流分组','Manage workflow groups independently')}</span></div>
                <div class="panel-actions compact-actions">
                    ${workflowTreeEdit?.placement === 'head' ? '' : `<button class="asset-icon-btn" type="button" data-workflow-cat-new title="${L('新建工作流分组','New workflow group')}"><i data-lucide="folder-plus"></i></button>`}
                ...
                <div class="nav-tree">
                    <div class="tree-branch expanded">
                        <button class="tree-row tree-parent contains-active" type="button" data-workflow-root>
                            ...
                            <span class="tree-row-name">${L('工作流库','Workflow library')}</span>
                        ...
                        <div class="tree-children">
                            ${cats.length ? cats.map(c => {
                                ...
                                <span class="tree-row-name">${escapeHtml(c.name || L('工作流','Workflow'))}</span>
                                <span class="tree-row-count">${(c.items || []).length}</span>
                            </button>${active ? renderWorkflowTreeActionBar() : ''}`;
                            }).join('') : `<div class="tree-empty">${L('暂无工作流分组','No workflow groups')}</div>`}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="asset-panel asset-content ${workflowManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>${escapeHtml(cat?.name || L('工作流管理','Workflow management'))}</strong>
                    <span>${L(`工作流库 / ${items.length} 个工作流`,`Workflow library / ${items.length} workflows`)}</span>
                </div>
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="workflowSearch" class="asset-search" type="search" value="${escapeAttr(workflowQuery)}" placeholder="${L('搜索工作流','Search workflows')}"></label>
                    <button class="asset-btn ${workflowManageMode ? 'primary' : ''}" type="button" data-workflow-manage><i data-lucide="list-checks"></i><span>${workflowManageMode ? L('完成管理','Done') : L('批量管理','Batch manage')}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span>${L(`已选择 ${selectedWorkflowIds.size} 个工作流。`,`Selected ${selectedWorkflowIds.size} workflows.`)}</span>
                <div class="asset-tools">
                    <button class="asset-btn" type="button" data-workflow-select-all ${items.length ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                    <button class="asset-btn" type="button" data-workflow-clear-selection ${selectedWorkflowIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>${L('清空','Clear')}</span></button>
                    <button class="asset-btn" type="button" data-workflow-export-selected ${selectedWorkflowIds.size ? '' : 'disabled'}><i data-lucide="download"></i><span>${L('导出所选','Export selected')}</span></button>
                    <button class="asset-btn danger" type="button" data-workflow-delete-selected ${selectedWorkflowIds.size ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${L('删除所选','Delete selected')}</span></button>
                </div>
            </div>
            <div class="content-scroll">
                <div class="asset-grid">
                    ...
                    ${items.length ? '' : `<div class="empty-state">${L('当前分组还没有工作流，可以上传 JSON / ZIP，或从传统画布导出到资产库。','No workflows in this group yet. Upload JSON / ZIP, or export from classic canvas.')}</div>`}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">
...
function renderWorkflowUploadCard(cat){
    return `<button id="workflowDrop" class="upload-grid-card" type="button" data-workflow-upload ${!cat ? 'disabled' : ''}>
        <span class="upload-thumb"><i data-lucide="upload-cloud"></i></span>
        <span class="upload-body">
            <strong>${L('上传工作流','Upload workflow')}</strong>
            <small>${L('支持 JSON / ZIP','Supports JSON / ZIP')}</small>
        </span>
    </button>`;
}
function renderWorkflowTreeActionBar(){
    ...
    return `<div class="tree-action-bar child-actions">
        <button type="button" data-workflow-cat-rename><i data-lucide="pencil"></i><span>${L('重命名','Rename')}</span></button>
        <button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-workflow-cat-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? L('确认删除','Confirm delete') : L('删除','Delete')}</span></button>
    </div>`;
}
function renderWorkflowTreeInlineEdit(){
    ...
    const label = workflowTreeEdit.label || L('名称','Name');
    return `<div class="tree-inline-edit child-actions">
        ...
        <button type="button" class="primary" data-workflow-tree-edit-save><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
        <button type="button" data-workflow-tree-edit-cancel><i data-lucide="x"></i><span>${L('取消','Cancel')}</span></button>
    </div>`;
}
...
function renderWorkflowDetail(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>${L('工作流详情','Workflow detail')}</strong><span>${L('选择一个工作流查看详情','Select a workflow to view detail')}</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="workflow"></i><span>${L('暂无工作流','No workflows')}</span></div></div>`;
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>${L('工作流详情','Workflow detail')}</strong><span>${escapeHtml(workflowKindLabel(item))}</span></div>
            <div class="panel-actions">
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                ...
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(workflowKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>${L('创建时间','Created')}</span><strong>${escapeHtml(formatDate(item.created_at))}</strong></div>
                    <div class="detail-meta"><span>${L('位置','Location')}</span><strong>${L('工作流库','Workflow library')}</strong></div>
                    <div class="detail-meta"><span>${L('分组','Group')}</span><strong>${escapeHtml(activeWorkflowCategory()?.name || L('工作流','Workflow'))}</strong></div>
                </div>
                <div class="detail-url">${escapeHtml(item.url || '')}</div>
            </div>
        </div>
...
function renderUploadCard(cat){
    return `<button id="assetDrop" class="upload-grid-card" type="button" data-asset-upload ${!cat ? 'disabled' : ''}>
        <span class="upload-thumb"><i data-lucide="upload-cloud"></i></span>
        <span class="upload-body">
            <strong>${L('上传到当前分组','Upload to current group')}</strong>
            <small>${L('拖入文件或点击上传','Drag file or click to upload')}</small>
        </span>
    </button>`;
}
function renderAssetClipboardBar(){
    ...
    const modeLabel = assetClipboard.mode === 'cut' ? L('剪切','Cut') : L('复制','Copy');
    ...
    const pasteText = sameTarget && assetClipboard.mode === 'cut' ? L('选择其他分组后粘贴','Select another group to paste') : L('粘贴到当前分组','Paste to current group');
    return `<div class="asset-clipboard-bar">
        <div class="asset-clipboard-info"><i data-lucide="clipboard"></i><span>${L(`${escapeHtml(modeLabel)}了 ${assetClipboard.ids.length} 个素材`,`${escapeHtml(modeLabel)} ${assetClipboard.ids.length} assets`)}</span></div>
        <div class="asset-tools">
            ...
            <button class="asset-icon-btn" type="button" data-asset-clear-clipboard title="${L('清空剪贴板','Clear clipboard')}"><i data-lucide="x"></i></button>
        </div>
    </div>`;
}
function renderAssetTreeBranch(lib){
    ...
    return `<div class="tree-branch ${isActiveLib ? 'expanded' : ''}">
        <button class="tree-row tree-parent ${isActiveLib ? 'contains-active' : ''} ${showLibActions ? 'active' : ''}" type="button" data-asset-lib="${escapeAttr(lib.id)}">
            ...
            <span class="tree-row-name">${escapeHtml(lib.name || L('资产库','Asset library'))}</span>
        ...
        <div class="tree-children">
            ${cats.length ? cats.map(cat => `<button class="tree-row tree-child ${isActiveLib && cat.id === activeAssetCategoryId && assetTreeFocus === 'category' ? 'active' : ''}" type="button" data-asset-cat="${escapeAttr(cat.id)}" data-asset-cat-lib="${escapeAttr(lib.id)}">
                ...
                <span class="tree-row-name">${escapeHtml(cat.name || L('分组','Group'))}</span>
                <span class="tree-row-count">${mediaAssetItems(cat.items).length}</span>
            </button>${isActiveLib && cat.id === activeAssetCategoryId && assetTreeFocus === 'category' ? renderAssetTreeActionBar('category') : ''}`).join('') : `<div class="tree-empty">${L('暂无分组','No groups')}</div>`}
...
function renderAssetTreeActionBar(kind){
    ...
    if(kind === 'library'){
        return `<div class="tree-action-bar library-actions">
            <button type="button" data-asset-cat-new><i data-lucide="folder-plus"></i><span>${L('新分组','New group')}</span></button>
            <button type="button" data-asset-lib-rename><i data-lucide="pencil"></i><span>${L('重命名','Rename')}</span></button>
            <button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-asset-lib-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? L('确认删除','Confirm delete') : L('删除库','Delete library')}</span></button>
        </div>`;
    }
    return `<div class="tree-action-bar child-actions">
        <button type="button" data-asset-cat-new><i data-lucide="folder-plus"></i><span>${L('新分组','New group')}</span></button>
        <button type="button" data-asset-cat-rename><i data-lucide="pencil"></i><span>${L('重命名','Rename')}</span></button>
        <button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-asset-cat-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? L('确认删除','Confirm delete') : L('删除','Delete')}</span></button>
    </div>`;
}
function renderAssetTreeInlineEdit(kind){
    ...
    const label = assetTreeEdit.label || L('名称','Name');
    return `<div class="tree-inline-edit ${kind === 'category' ? 'child-actions' : 'library-actions'}">
        ...
        <button type="button" class="primary" data-asset-tree-edit-save><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
        <button type="button" data-asset-tree-edit-cancel><i data-lucide="x"></i><span>${L('取消','Cancel')}</span></button>
    </div>`;
}
...
function renderAvatarRegistrationCard(item, platform, reg, busy){
    ...
    if(status === 'Active' && reg.asset_uri){
        return `<div class="avatar-card registered">
            <div class="avatar-head"><i data-lucide="badge-check"></i><span>${L('已认证可用','Verified & available')}</span>${tag}</div>
    ...
    if(status === 'Processing'){
        return `<div class="avatar-card processing">
            <div class="avatar-head"><i data-lucide="loader"></i><span>${L('审核中','Under review')}</span>${tag}</div>
            <div class="avatar-hint">${L(`已提交到 ${escapeHtml(avatarPlatformLabel(platform))} 审核（任务 ${escapeHtml(reg.task_id || '')}），通过后会自动生成 asset:// 地址。审核通常需要几十秒到几分钟。`,`Submitted to ${escapeHtml(avatarPlatformLabel(platform))} review (task ${escapeHtml(reg.task_id || '')}); an asset:// address is generated automatically once approved. Review usually takes tens of seconds to minutes.`)}</div>
            <div class="asset-tools">
                <button class="asset-btn primary" type="button" data-avatar-check="${escapeAttr(item.id)}" ${provAttr} ${busy ? 'disabled' : ''}><i data-lucide="refresh-cw"></i><span>${busy ? L('查询中…','Checking…') : L('刷新审核状态','Refresh review status')}</span></button>
            </div>
        </div>`;
    }
    return `<div class="avatar-card failed">
        <div class="avatar-head"><i data-lucide="x-circle"></i><span>${L('审核未通过','Review failed')}</span>${tag}</div>
        <div class="avatar-hint warn">${escapeHtml(reg.detail || L('审核未通过，请更换素材后重试。','Review failed; try a different asset.'))}</div>
        <div class="asset-tools">
...
function renderAvatarSection(item){
    ...
    if(!providers.length){
        return `<div class="avatar-section">
            ${cards}
            <div class="avatar-head"><i data-lucide="user-round-cog"></i><span>${L('注册为真人/数字人','Register as real person / avatar')}</span></div>
            <div class="avatar-hint">${L('未检测到可用平台。请先在「API 平台管理」中添加并启用 API 平台（如 APIMart）并填写 Key。','No available platform detected. Add and enable an API platform (e.g. APIMart) in "API Platforms" and fill in the key.')}</div>
        </div>`;
    }
    ...
    let registerUI;
    if(!supported){
        registerUI = `<div class="avatar-hint">${L(`认证是跨平台功能，但「${escapeHtml(selPlatform ? avatarPlatformLabel(selPlatform) : (selected?.name || selected?.id || L('该平台','this platform')))}」的资产认证 API 尚未接入（待接入）。请选择已支持的平台，或继续使用官方控制台认证。`,`Authentication is cross-platform, but the asset auth API for "${escapeHtml(selPlatform ? avatarPlatformLabel(selPlatform) : (selected?.name || selected?.id || 'this platform'))}" is not yet integrated. Choose a supported platform or continue using the official console.`)}</div>${select}`;
    } else {
        registerUI = `
            <div class="avatar-hint">${L(`提交到 ${escapeHtml(avatarPlatformLabel(selPlatform))} 私域素材审核，通过后生成 asset:// 地址，可在该平台的视频生成中通过 @ 直接调用（一个素材可注册到多个平台，平台间互相隔离）。`,`Submit to ${escapeHtml(avatarPlatformLabel(selPlatform))} private asset review; an asset:// address is generated once approved, callable via @ in that platform's video generation (one asset can register to multiple isolated platforms).`)}</div>
            ${select}
            ${noKey ? `<div class="avatar-hint warn">${L('该平台尚未配置 API Key。','This platform has no API key configured.')}</div>` : ''}
            ${alreadyRegistered ? `<div class="avatar-hint">${L('该平台已注册，再次提交会覆盖该平台的认证。','Already registered on this platform; resubmitting overwrites it.')}</div>` : ''}
            <button class="asset-btn primary" type="button" data-avatar-register="${escapeAttr(item.id)}" data-avatar-prov="${escapeAttr(selected?.id || '')}" ${busy || noKey ? 'disabled' : ''}><i data-lucide="user-round-plus"></i><span>${busy ? L('注册中，请稍候…','Registering, please wait…') : (alreadyRegistered ? L('重新注册到该平台','Re-register on this platform') : L('注册并等待审核','Register and await review'))}</span></button>`;
    }
    return `<div class="avatar-section">
        ${cards}
        <div class="avatar-head"><i data-lucide="user-round-cog"></i><span>${L('注册到平台','Register to platform')}</span></div>
        ${registerUI}
    </div>`;
}
function renderAssetDetail(item){
    ...
    if(assetEditMode && item.id === selectedAssetId){
        return `
            <div class="panel-head">
                <div class="panel-title"><strong>${L('编辑素材','Edit asset')}</strong><span>${L('当前分组内直接保存','Save directly in current group')}</span></div>
                <div class="panel-actions">
                    <button class="asset-btn primary" type="button" data-asset-edit-save="${escapeAttr(item.id)}"><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
                    <button class="asset-icon-btn" type="button" data-asset-edit-cancel title="${L('取消','Cancel')}"><i data-lucide="x"></i></button>
                </div>
            </div>
            <div class="detail-scroll">
                ...
                <div class="inline-edit-form">
                    <label class="inline-edit-field"><span>${L('素材名称','Asset name')}</span><input id="assetEditName" type="text" value="${escapeAttr(item.name || '')}" placeholder="${L('素材名称','Asset name')}"></label>
                    <div class="detail-meta-grid">
                        <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(assetKindLabel(item))}</strong></div>
                        <div class="detail-meta"><span>${L('创建时间','Created')}</span><strong>${escapeHtml(formatDate(item.created_at))}</strong></div>
                    </div>
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                ...
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>${L('类型','Type')}</span><strong>${escapeHtml(assetKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>${L('创建时间','Created')}</span><strong>${escapeHtml(formatDate(item.created_at))}</strong></div>
                    <div class="detail-meta"><span>${L('资产库','Asset library')}</span><strong>${escapeHtml(activeAssetLibrary()?.name || L('资产库','Asset library'))}</strong></div>
                    <div class="detail-meta"><span>${L('分组','Group')}</span><strong>${escapeHtml(activeAssetCategory()?.name || L('分组','Group'))}</strong></div>
                </div>
                <div class="detail-url">${escapeHtml(item.url || '')}</div>
                ${isImage ? `<div class="detail-caption-card">
                    <div class="detail-caption-head"><strong>${L('智能分类','Smart classify')}</strong></div>
                    <div class="detail-classification-body">${renderClassificationChips(item, 32, {kind:'asset', libraryId:activeAssetLibraryId}) || `<span class="classification-empty">${L('暂无智能分类，可以选中图片后点击"智能分类"。','No smart classification yet. Select images and click "Smart Classify".')}</span>`}</div>
                </div>` : ''}
...
function renderPromptManager(){
    ...
    const promptEmptyText = (lib?.items || []).length
        ? L('当前条件下没有提示词。可以切换分类或清空搜索条件。','No prompts under current filter. Switch category or clear search.')
        : L(`${lib?.name || L('当前提示词库','current library')} 暂无提示词，点击「新增」添加。`,`${lib?.name || 'current library'} has no prompts; click "Add" to create.`);
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>${L('提示词库','Prompt library')}</strong><span>${L('可创建多个词库','Create multiple libraries')}</span></div>
                <div class="panel-actions compact-actions">
                    ${promptTreeEdit?.placement === 'head' ? '' : `<button class="asset-icon-btn" type="button" data-prompt-lib-new title="${L('新建提示词库','New prompt library')}"><i data-lucide="plus"></i></button>`}
        ...
        <section class="asset-panel asset-content ${promptManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>${escapeHtml(lib?.name || L('提示词库','Prompt library'))}</strong>
                    <span>${L(`共 ${items.length} 条提示词`,`${items.length} prompts`)}</span>
                </div>
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="promptSearch" class="asset-search" type="search" value="${escapeAttr(promptQuery)}" placeholder="${L('搜索名称、说明或正文','Search name, description or body')}"></label>
                    <button class="asset-btn primary" type="button" data-prompt-new ${readonly ? 'disabled' : ''}><i data-lucide="file-plus-2"></i><span>${L('新增','Add')}</span></button>
                    <button class="asset-btn ${promptManageMode ? 'primary' : ''}" type="button" data-prompt-manage><i data-lucide="list-checks"></i><span>${promptManageMode ? L('完成管理','Done') : L('批量管理','Batch manage')}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span>${L(`已选择 ${selectedPromptIds.size} 条提示词，支持拖拽框选或逐个勾选。`,`Selected ${selectedPromptIds.size} prompts. Drag-select or tick individually.`)}</span>
                <div class="asset-tools">
                    <button class="asset-btn" type="button" data-prompt-select-all ${items.length && !readonly ? '' : 'disabled'}><i data-lucide="check-square"></i><span>${L('全选','Select all')}</span></button>
                    <button class="asset-btn" type="button" data-prompt-clear-selection ${selectedPromptIds.size ? '' : 'disabled'}><i data-lucide="square"></i><span>${L('清空','Clear')}</span></button>
                    <button class="asset-btn danger ${pendingBatchDelete === 'prompt' ? 'detail-confirm' : ''}" type="button" data-prompt-delete-selected ${readonly || !selectedPromptIds.size ? 'disabled' : ''}><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'prompt' ? L('确认删除','Confirm delete') : L('删除所选','Delete selected')}</span></button>
                </div>
            </div>
            <div class="content-scroll">
...
function renderPromptTreeBranch(lib){
    ...
    return `<div class="tree-branch ${isActiveLib ? 'expanded' : ''}">
        <button class="tree-row tree-parent ${isActiveLib ? 'contains-active' : ''} ${showLibActions ? 'active' : ''}" type="button" data-prompt-lib="${libId}">
            ...
            <span class="tree-row-name">${escapeHtml(lib.name || L('提示词库','Prompt library'))}</span>
        ...
        <div class="tree-children">
            <button class="tree-row tree-child ${isActiveLib && activePromptCategory === 'all' && promptTreeFocus === 'category' ? 'active' : ''}" type="button" data-prompt-cat="all" data-prompt-cat-lib="${libId}">
                ...
                <span class="tree-row-name">${L('全部提示词','All prompts')}</span>
...
function renderPromptTreeActionBar(kind){
    ...
    if(kind === 'library'){
        ...
        return `<div class="tree-action-bar library-actions">
            <button type="button" data-prompt-cat-new><i data-lucide="folder-plus"></i><span>${L('新分组','New group')}</span></button>
            <button type="button" data-prompt-lib-rename><i data-lucide="pencil"></i><span>${L('重命名','Rename')}</span></button>
            ${isSystem ? '' : `<button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-prompt-lib-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? L('确认删除','Confirm delete') : L('删除库','Delete library')}</span></button>`}
        </div>`;
    }
    if(activePromptCategory === 'all'){
        return `<div class="tree-action-bar child-actions muted-actions"><span><i data-lucide="lock"></i>${L('请选择具体分组后编辑','Select a specific group to edit')}</span></div>`;
    }
    const deleteKey = `prompt-cat:${activePromptCategory}`;
    return `<div class="tree-action-bar child-actions">
        <button type="button" data-prompt-cat-rename><i data-lucide="pencil"></i><span>${L('重命名','Rename')}</span></button>
        <button type="button" class="danger ${pendingTreeDelete === deleteKey ? 'detail-confirm' : ''}" data-prompt-cat-delete><i data-lucide="trash-2"></i><span>${pendingTreeDelete === deleteKey ? L('确认删除','Confirm delete') : L('删除','Delete')}</span></button>
    </div>`;
}
function renderPromptTreeInlineEdit(kind){
    ...
    const label = promptTreeEdit.label || L('名称','Name');
    return `<div class="tree-inline-edit ${kind === 'category' ? 'child-actions' : 'library-actions'}">
        <input id="promptTreeEditInput" type="text" value="${escapeAttr(promptTreeEdit.value || '')}" placeholder="${escapeAttr(label)}">
        <button type="button" class="primary" data-prompt-tree-edit-save><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
        <button type="button" data-prompt-tree-edit-cancel><i data-lucide="x"></i><span>${L('取消','Cancel')}</span></button>
    </div>`;
}
function renderPromptRow(item, readonly){
    return `<article class="prompt-row ${item.id === selectedPromptId ? 'active' : ''}" data-prompt-row="${escapeAttr(item.id)}">
        ...
        <div class="prompt-row-main">
            <div class="prompt-row-title"><strong>${escapeHtml(item.name || L('提示词','Prompt'))}</strong><span class="prompt-tag">${escapeHtml(promptCategoryLabel(item.category || 'custom'))}</span></div>
            <div class="prompt-row-scene">${escapeHtml(item.scene || L('未填写用途说明','No usage description'))}</div>
...
function renderPromptDetail(item, readonly){
    if(promptCreateMode && !readonly){
        return `
            <div class="panel-head">
                <div class="panel-title"><strong>${L('新增提示词','Add prompt')}</strong><span>${L('保存到当前提示词库','Save to current prompt library')}</span></div>
                <div class="panel-actions">
                    <button class="asset-btn primary" type="button" data-prompt-create-save><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
                    <button class="asset-icon-btn" type="button" data-prompt-edit-cancel title="${L('取消','Cancel')}"><i data-lucide="x"></i></button>
                </div>
            </div>
            <div class="detail-scroll">
                <div class="inline-edit-form">
                    <label class="inline-edit-field"><span>${L('名称','Name')}</span><input id="promptEditName" type="text" value="" placeholder="${L('提示词名称','Prompt name')}"></label>
                    <label class="inline-edit-field"><span>${L('用途说明','Usage description')}</span><textarea id="promptEditScene" placeholder="${L('用途说明','Usage description')}"></textarea></label>
                    <label class="inline-edit-field"><span>${L('正向提示词','Positive prompt')}</span><textarea id="promptEditPositive" placeholder="${L('正向提示词','Positive prompt')}"></textarea></label>
                    <label class="inline-edit-field"><span>${L('负向提示词','Negative prompt')}</span><textarea id="promptEditNegative" placeholder="${L('负向提示词','Negative prompt')}"></textarea></label>
                </div>
            </div>
        `;
    }
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>${L('提示词预览','Prompt preview')}</strong><span>${L('选择一条提示词查看全文','Select a prompt to view full text')}</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="text-cursor-input"></i><span>${L('暂无可预览提示词','No previewable prompt')}</span></div></div>`;
    if(promptEditMode && item.id === selectedPromptId && !readonly){
        return `
            <div class="panel-head">
                <div class="panel-title"><strong>${L('编辑提示词','Edit prompt')}</strong><span>${L('在当前库内保存','Save within current library')}</span></div>
                <div class="panel-actions">
                    <button class="asset-btn primary" type="button" data-prompt-edit-save="${escapeAttr(item.id)}"><i data-lucide="check"></i><span>${L('保存','Save')}</span></button>
                    <button class="asset-icon-btn" type="button" data-prompt-edit-cancel title="${L('取消','Cancel')}"><i data-lucide="x"></i></button>
                </div>
            </div>
            <div class="detail-scroll">
                <div class="inline-edit-form">
                    <label class="inline-edit-field"><span>${L('名称','Name')}</span><input id="promptEditName" type="text" value="${escapeAttr(item.name || '')}" placeholder="${L('提示词名称','Prompt name')}"></label>
                    <label class="inline-edit-field"><span>${L('用途说明','Usage description')}</span><textarea id="promptEditScene" placeholder="${L('用途说明','Usage description')}">${escapeHtml(item.scene || '')}</textarea></label>
                    <label class="inline-edit-field"><span>${L('正向提示词','Positive prompt')}</span><textarea id="promptEditPositive" placeholder="${L('正向提示词','Positive prompt')}">${escapeHtml(item.positive || '')}</textarea></label>
                    <label class="inline-edit-field"><span>${L('负向提示词','Negative prompt')}</span><textarea id="promptEditNegative" placeholder="${L('负向提示词','Negative prompt')}">${escapeHtml(item.negative || '')}</textarea></label>
                </div>
            </div>
        `;
    }
    ...
    return `
        <div class="panel-head">
            <div class="panel-title"><strong>${L('提示词预览','Prompt preview')}</strong><span>${escapeHtml(promptCategoryLabel(item.category || 'custom'))}</span></div>
            <div class="panel-actions">
        ...
        <div class="detail-scroll">
            <div class="prompt-detail-head">
                <div class="prompt-detail-title">${escapeHtml(item.name || L('提示词','Prompt'))}</div>
                <div class="prompt-detail-scene">${escapeHtml(item.scene || L('未填写用途说明','No usage description'))}</div>
            </div>
            <section class="prompt-block">
                <div class="prompt-block-head"><span>${L('正向提示词','Positive prompt')}</span><span>${L(`${String(item.positive || '').length} 字符`,`${String(item.positive || '').length} chars`)}</span></div>
                <textarea class="prompt-block-body" readonly spellcheck="false">${escapeHtml(item.positive || L('未填写','Not filled in'))}</textarea>
            </section>
            <section class="prompt-block">
                <div class="prompt-block-head"><span>${L('负向提示词','Negative prompt')}</span><span>${L(`${String(item.negative || '').length} 字符`,`${String(item.negative || '').length} chars`)}</span></div>
                <textarea class="prompt-block-body negative" readonly spellcheck="false">${escapeHtml(item.negative || L('未填写','Not filled in'))}</textarea>
            </section>
...
async function uploadFiles(files){
    const cat = activeAssetCategory();
    if(!cat) throw new Error(L('请先创建图片分组','Create an image group first'));
    ...
    if(!items.length) throw new Error(L('没有可保存的素材','No assets to save'));
    ...
    render();
    setStatus(L(`已上传 ${items.length} 个素材`,`Uploaded ${items.length} assets`));
    return {count:items.length, items:data.items || []};
}
async function uploadWorkflowFiles(files){
    const cat = activeWorkflowCategory();
    if(!cat) throw new Error(L('请先创建工作流分组','Create a workflow group first'));
    ...
    if(!list.length) throw new Error(L('没有可上传的工作流文件','No workflow file to upload'));
    const form = new FormData();
    ...
    render();
    setStatus(L(`已上传 ${data.items?.length || 0} 个工作流`,`Uploaded ${data.items?.length || 0} workflows`));
}
...
async function exportWorkflowItems(ids){
    ...
    if(items.length === 1){
        ...
        setStatus(L('已导出工作流','Workflow exported'));
        return;
    }
    ...
    if(!res.ok){ const ed = await res.json().catch(() => ({})); throw new Error(ed.detail || ed.error || L('导出工作流失败','Export workflow failed')); }
    ...
    setStatus(L(`已导出 ${items.length} 个工作流`,`Exported ${items.length} workflows`));
}
async function downloadCanvasAssetItems(ids){
    const items = (ids || []).map(id => findCanvasAssetItem(id)).filter(item => item?.url);
    if(!items.length){
        setStatus(L('没有可下载的画布资产','No downloadable canvas assets'));
        return;
    }
    if(items.length === 1){
        const item = items[0];
        downloadUrl(item.url, item.name || 'canvas-asset');
        setStatus(L('已下载画布资产','Canvas assets downloaded'));
        return;
    }
    setStatus(items.length === 1 ? L('正在下载画布资产...','Downloading canvas asset...') : L(`正在打包 ${items.length} 个画布资产...`,`Packing ${items.length} canvas assets...`));
    const res = await fetch('/api/canvas-assets/download', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({filename:'canvas-assets.zip', items:items.map(item => ({url:item.url, name:item.name || 'canvas-asset'}))})
    });
    if(!res.ok){ const ed = await res.json().catch(() => ({})); throw new Error(ed.detail || ed.error || L('下载画布资产失败','Download canvas asset failed')); }
    const blob = await res.blob();
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    ...
    link.remove();
    setTimeout(() => URL.revokeObjectURL(link.href), 1200);
    setStatus(L(`已下载 ${items.length} 个画布资产`,`Downloaded ${items.length} canvas assets`));
}
...
function downloadAssetItem(id){
    ...
    if(!item || !item.url){ setStatus(L('没有可下载的素材','No downloadable asset')); return; }
    ...
    setStatus(L('已开始下载','Download started'));
}
const SMART_CANVAS_ASSET_INBOX_KEY = 'smart_canvas_asset_inbox';
function canvasInboxAssetFromItem(item){
    const out = {url:item?.url || '', name:item?.name || L('素材','Asset'), kind:item?.kind || ''};
...
function copySelectedAssetsToCanvas(){
    ...
    if(!items.length){ setStatus(L('没有可复制的素材','No copyable asset')); return; }
    try {
        // 写入跨页剪贴板，画布页按 Ctrl+V 读取并批量粘贴
        localStorage.setItem(SMART_CANVAS_ASSET_INBOX_KEY, JSON.stringify({items, ts: Date.now()}));
    } catch(err){
        setStatus(L('复制失败：','Copy failed: ') + (err?.message || err));
        return;
    }
    setStatus(L(`已复制 ${items.length} 个素材，去智能画布按 Ctrl+V 粘贴`,`Copied ${items.length} assets. Paste with Ctrl+V in Smart Canvas`));
}
async function downloadSelectedAssets(){
    const items = [...selectedAssetIds].map(id => findAssetItem(id)).filter(it => it?.url);
    if(!items.length){ setStatus(L('没有可下载的素材','No downloadable asset')); return; }
    if(items.length === 1){ downloadAssetItem(items[0].id); return; }
    setStatus(L(`正在打包 ${items.length} 个素材...`,`Packing ${items.length} assets...`));
    try {
        const res = await fetch('/api/canvas-assets/download', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
        ...
        if(!res.ok){ const ed = await res.json().catch(() => ({})); throw new Error(ed.detail || ed.error || L('下载失败','Download failed')); }
        const blob = await res.blob();
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        ...
        link.remove();
        setTimeout(() => URL.revokeObjectURL(link.href), 1200);
        setStatus(L(`已下载 ${items.length} 个素材`,`Downloaded ${items.length} assets`));
    } catch(err){
        setStatus(err.message || L('下载失败','Download failed'));
    }
}
...
function renderLocalUploadClipboardBar(){
    ...
    const hereLabel = activeLocalUploadFolder ? localUploadFolderTitle() : L('根目录','Root');
    return `<div class="asset-clipboard-bar">
        <div class="asset-clipboard-info"><i data-lucide="clipboard"></i><span>${L(`已剪切 ${localUploadClipboard.ids.length} 个素材`,`Cut ${localUploadClipboard.ids.length} assets`)}</span></div>
        <div class="asset-tools">
            <button class="asset-btn primary" type="button" data-localup-paste-clipboard ${sameFolder ? 'disabled' : ''}><i data-lucide="clipboard-paste"></i><span>${sameFolder ? L('切到其他文件夹后移动','Switch to another folder to move') : L(`移动到「${escapeHtml(hereLabel)}」`,`Move to "${escapeHtml(hereLabel)}"`)}</span></button>
            <button class="asset-icon-btn" type="button" data-localup-clear-clipboard title="${L('取消剪切','Cancel cut')}"><i data-lucide="x"></i></button>
        </div>
    </div>`;
}
function setLocalUploadClipboard(){
    const ids = [...selectedLocalUploadIds];
    if(!ids.length){ setStatus(L('没有选中的素材','No selected asset')); return; }
    localUploadClipboard = {ids, sourceFolder: activeLocalUploadFolder || ''};
    selectedLocalUploadIds.clear();
    render();
    setStatus(L(`已剪切 ${ids.length} 个素材，切换到目标文件夹后点"移动到此文件夹"`,`Cut ${ids.length} assets. Switch to target folder then click "Move here"`));
}
async function pasteLocalUploadClipboard(){
    ...
    try {
        ...
        selectedLocalUploadIds.clear();
        render();
        setStatus(L(`已移动 ${data?.moved ?? names.length} 个素材`,`Moved ${data?.moved ?? names.length} assets`));
    } catch(err){
        setStatus(err.message || L('移动失败','Move failed'));
    }
}
async function renameWorkflowItem(id){
    const item = findWorkflowItem(id);
    const name = window.prompt(L('工作流名称','Workflow name'), item?.name || '');
    ...
    assetLibrary = data.library || assetLibrary;
    render();
    setStatus(L('已重命名工作流','Workflow renamed'));
}
async function deleteWorkflowItem(id){
    ...
    if(pendingDeleteAssetId !== id){
        ...
        render();
        setStatus(L('再次点击确认删除工作流','Click again to confirm deleting workflow'));
        return;
    }
    ...
    render();
    setStatus(L('已删除工作流','Workflow deleted'));
}
async function deleteSelectedWorkflows(){
    ...
    selectedWorkflowId = '';
    render();
    setStatus(L(`已删除 ${data.removed || 0} 个工作流`,`Deleted ${data.removed || 0} workflows`));
}
async function uploadLocalAssets(files){
    const list = [...files].filter(f => isLocalMediaFile(f));
    if(!list.length){ setStatus(L('没有可上传的图片/视频/音频文件','No uploadable image/video/audio file')); return; }
    const form = new FormData();
    form.append('folder', activeLocalUploadFolder || '');
    list.forEach(file => form.append('files', file));
    setStatus(L('正在上传...','Uploading...'));
    try {
        ...
        render();
        setStatus(L(`已上传 ${uploaded.length} 个素材`,`Uploaded ${uploaded.length} assets`));
    } catch(err) {
        setStatus(err.message || L('上传失败','Upload failed'));
    }
}
async function deleteLocalAssets(ids){
    const names = (ids || []).map(id => findLocalUpload(id)?.file).filter(Boolean);
    if(!names.length) return;
    setStatus(L('正在删除...','Deleting...'));
    try {
        ...
        if(selectedLocalUploadId && !findLocalUpload(selectedLocalUploadId)) selectedLocalUploadId = '';
        render();
        setStatus(L(`已删除 ${names.length} 个素材`,`Deleted ${names.length} assets`));
    } catch(err) {
        setStatus(err.message || L('删除失败','Delete failed'));
    }
}
async function saveLocalUploadInlineName(id, name){
    ...
    try {
        ...
        render();
        setStatus(L('已重命名素材，反推提示词和分类索引已同步','Asset renamed; caption and classification index synced'));
    } catch(err) {
        setStatus(err.message || L('重命名失败','Rename failed'));
    }
}
function beginLocalUploadInlineRename(id){
    ...
    const previousName = item.name || L('本地素材','Local asset');
    const input = document.createElement('input');
    input.className = 'asset-card-name-input';
    input.type = 'text';
    input.value = previousName;
    input.setAttribute('aria-label', L('素材名称','Asset name'));
...
async function createLocalUploadFolder(){
    const name = window.prompt(L('新建文件夹名称','New folder name'), L('新文件夹','New folder'));
    if(!String(name || '').trim()) return;
    try {
        ...
        selectedLocalUploadIds.clear();
        render();
        setStatus(L('已新建本地素材文件夹','Local asset folder created'));
    } catch(err) {
        setStatus(err.message || L('新建文件夹失败','Create folder failed'));
    }
}
async function renameLocalUploadFolder(){
    if(!activeLocalUploadFolder){
        setStatus(L('根目录不能重命名','Root folder cannot be renamed'));
        return;
    }
    const current = localUploadFolderTitle();
    const name = window.prompt(L('重命名文件夹','Rename folder'), current);
    ...
    try {
        ...
        selectedLocalUploadIds.clear();
        render();
        setStatus(L('已重命名本地素材文件夹','Local asset folder renamed'));
    } catch(err) {
        setStatus(err.message || L('重命名文件夹失败','Rename folder failed'));
    }
}
async function runLocalUploadCaptionSelected(){
    ...
    if(!localCaptionProvider || !localCaptionModel){
        setStatus(L('请先在 API 设置中配置可用的聊天/视觉模型','Configure a chat/vision model in API settings first'));
        return;
    }
    localCaptionBusy = true;
    render();
    setStatus(L(`正在反推 ${images.length} 张本地图片的提示词...`,`Reversing captions for ${images.length} local images...`));
    try {
        const data = await apiJson('/api/local-assets/caption', {
            ...
            body:JSON.stringify({
                ...
                provider:localCaptionProvider,
                model:localCaptionModel,
                prompt:(localCaptionPrompt || L('描述图片','Describe the image')).trim() || L('描述图片','Describe the image')
            })
        });
        await loadLocalAssets();
        selectedLocalUploadIds.clear();
        ...
        setStatus(failed.length ? L(`已完成 ${data.count || 0} 张，${failed.length} 张失败：${failed[0].error || L('反推失败','reverse failed')}`,`Done ${data.count || 0}, ${failed.length} failed: ${failed[0].error || 'reverse failed'}`) : L(`已反推并保存 ${data.count || images.length} 张图片提示词`,`Reversed and saved captions for ${data.count || images.length} images`));
    } catch(err) {
        setStatus(err.message || L('提示词反推失败','Caption reverse failed'));
    } finally {
...
async function runLocalUploadCaptionOne(id){
    ...
    render();
    setStatus(L(`正在反推「${item.name || L('本地图片','Local image')}」的提示词...`,`Reversing caption for "${item.name || 'Local image'}"...`));
    try {
        const data = await apiJson('/api/local-assets/caption', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
        ...
        if(result && !result.ok) throw new Error(result.error || L('反推失败','Reverse failed'));
        await loadLocalAssets();
        selectedLocalUploadId = item.id;
        render();
        setStatus(L('已反推并保存当前图片提示词','Caption reversed and saved'));
    } catch(err) {
        setStatus(err.message || L('提示词反推失败','Caption reverse failed'));
    } finally {
        localCaptionBusy = false;
...
async function copyLocalUploadCaption(id){
    ...
    if(!String(text || '').trim()){
        setStatus(L('当前图片暂无可复制的提示词','No copyable caption for this image'));
        return;
    }
    const ok = await copyTextToClipboard(text);
    setStatus(ok ? L('已复制提示词','Caption copied') : L('复制失败，请手动复制','Copy failed, copy manually'));
}
async function runLocalUploadClassifySelected(){
    ...
    render();
    setStatus(L(`正在智能分类 ${images.length} 张本地图片...`,`Smart-classifying ${images.length} local images...`));
    try {
        ...
        render();
        const failed = (data.items || []).filter(item => !item.ok);
        setStatus(failed.length ? L(`处理完成 ${data.count || 0} 张，${failed.length} 张失败：${failed[0].error || L('分类失败','classification failed')}`,`Done ${data.count || 0}, ${failed.length} failed: ${failed[0].error || 'classification failed'}`) : L(`处理完成 ${data.count || images.length} 张图片`,`Processed ${data.count || images.length} images`));
    } catch(err) {
        setStatus(err.message || L('智能分类失败','Smart classification failed'));
    } finally {
...
async function runAssetClassifySelected(){
    ...
    render();
    setStatus(L(`正在智能分类 ${images.length} 张资产图片...`,`Smart-classifying ${images.length} asset images...`));
    try {
        ...
        render();
        const failed = (data.items || []).filter(item => !item.ok);
        setStatus(failed.length ? L(`处理完成 ${data.count || 0} 张，${failed.length} 张失败：${failed[0].error || L('分类失败','classification failed')}`,`Done ${data.count || 0}, ${failed.length} failed: ${failed[0].error || 'classification failed'}`) : L(`处理完成 ${data.count || images.length} 张资产图片`,`Processed ${data.count || images.length} asset images`));
    } catch(err) {
        setStatus(err.message || L('智能分类失败','Smart classification failed'));
    } finally {
...
async function saveLocalUploadCaption(id){
    ...
    try {
        ...
        render();
        setStatus(L('已保存反推提示词','Caption saved'));
    } catch(err) {
        setStatus(err.message || L('保存提示词失败','Save caption failed'));
    }
}
async function handleClick(event){
    ...
    if(prefEditorSaveBtn){
        try {
            ...
            setStatus(err.message || L('保存偏好设置失败','Save preferences failed'));
        }
        return;
    }
    ...
    if(target.closest?.('[data-storage-save]')){
        ...
        catch(err){ setStatus(err.message || L('保存存储设置失败','Save storage settings failed')); }
        return;
    }
    ...
    if(target.closest?.('[data-storage-delete]')){
        ...
        catch(err){ setStatus(err.message || L('删除文件失败','Delete file failed')); }
        return;
    }
    ...
    if(target.closest?.('[data-localup-class-root]')){
        if(!localUploadClassEntries().length) setStatus(L('暂无智能分类，请先选择图片并点击"智能分类"','No smart classification yet. Select images and click "Smart Classify"'));
        return;
    }
    ...
    if(localUpCopy){ const it = findLocalUpload(localUpCopy.dataset.localupCopy || ''); const ok = await copyTextToClipboard(it?.url || ''); setStatus(ok ? L('已复制链接','Link copied') : L('复制失败','Copy failed')); return; }
    const localUpCard = target.closest?.('[data-localup-card]');
    if(localUpCard){
    ...
    const canvasAssetScopeBtn = target.closest?.('[data-canvas-asset-scope]');
    if(canvasAssetScopeBtn){ canvasAssetScope = canvasAssetScopeBtn.dataset.canvasAssetScope || 'all'; canvasAssetShowTrash = false; selectedCanvasAssetId = ''; render(); return; }
    if(target.closest?.('[data-canvas-asset-trash]')){ canvasAssetShowTrash = !canvasAssetShowTrash; selectedCanvasAssetId = ''; selectedCanvasAssetIds.clear(); render(); return; }
    const canvasAssetFavorite = target.closest?.('[data-canvas-asset-favorite]');
    if(canvasAssetFavorite){ const id = canvasAssetFavorite.dataset.canvasAssetFavorite || ''; const item = findCanvasAssetItem(id); if(item) { await apiJson(`/api/canvas/asset-center/${encodeURIComponent(id)}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({favorite:!item.favorite})}); await refreshCanvasAssets(); } return; }
    const canvasAssetDelete = target.closest?.('[data-canvas-asset-delete]');
    if(canvasAssetDelete){ await apiJson(`/api/canvas/asset-center/${encodeURIComponent(canvasAssetDelete.dataset.canvasAssetDelete || '')}/delete`, {method:'POST', headers:{'Content-Type':'application/json'}, body:'{}'}); await refreshCanvasAssets(); return; }
    const canvasAssetRestore = target.closest?.('[data-canvas-asset-restore]');
    if(canvasAssetRestore){ await apiJson(`/api/canvas/asset-center/${encodeURIComponent(canvasAssetRestore.dataset.canvasAssetRestore || '')}/restore`, {method:'POST', headers:{'Content-Type':'application/json'}, body:'{}'}); await refreshCanvasAssets(); return; }
    ...
    if(canvasAssetCopy){ const it = findCanvasAssetItem(canvasAssetCopy.dataset.canvasAssetCopy || ''); const ok = await copyTextToClipboard(it?.url || ''); setStatus(ok ? L('已复制画布资产链接','Canvas asset link copied') : L('复制失败','Copy failed')); return; }
    ...
    if(target.closest?.('[data-workflow-cat-new]')){
        workflowTreeEdit = {kind:'category-new', placement:'head', value:L('新工作流分组','New workflow group'), label:L('工作流分组名称','Workflow group name')};
        pendingTreeDelete = '';
        render();
    ...
    if(target.closest?.('[data-workflow-cat-rename]')){
        const cat = activeWorkflowCategory();
        if(!cat) return;
        workflowTreeEdit = {kind:'category-rename', value:cat.name || '', label:L('工作流分组名称','Workflow group name'), categoryId:cat.id};
        pendingTreeDelete = '';
        render();
        focusTreeEditInput('workflowTreeEditInput');
        return;
    ...
    if(avatarCopy){
        ...
        setStatus(ok ? L('已复制 asset:// 地址','asset:// address copied') : L('复制失败，请手动复制：','Copy failed, copy manually: ') + `${uri}`);
        return;
    }
    ...
    if(target.closest?.('[data-asset-lib-new]')){ assetTreeFocus = 'library'; assetTreeEdit = {kind:'library-new', placement:'head', value:L('新资产库','New asset library'), label:L('资产库名称','Asset library name')}; render(); focusTreeEditInput('assetTreeEditInput'); return; }
    if(target.closest?.('[data-asset-lib-rename]')){
        ...
        assetTreeEdit = {kind:'library-rename', value:activeAssetLibrary()?.name || '', label:L('资产库名称','Asset library name')};
        pendingTreeDelete = '';
        render(); return;
    }
    ...
    if(target.closest?.('[data-asset-cat-new]')){
        ...
        assetTreeEdit = {kind:'category-new', value:L('新分组','New group'), label:L('分组名称','Group name')};
        pendingTreeDelete = '';
        render(); return;
    }
    if(target.closest?.('[data-asset-cat-rename]')){
        ...
        assetTreeEdit = {kind:'category-rename', value:activeAssetCategory()?.name || '', label:L('分组名称','Group name')};
        pendingTreeDelete = '';
        render(); return;
    }
    ...
    if(assetClassRoot){
        ...
        if(!assetClassificationEntriesForLibrary(activeAssetLibrary()).length) setStatus(L('暂无智能分类，请先选择图片并点击"智能分类"','No smart classification yet. Select images and click "Smart Classify"'));
        return;
    }
    ...
    if(target.closest?.('[data-prompt-lib-new]')){ promptTreeFocus = 'library'; promptTreeEdit = {kind:'library-new', placement:'head', value:L('新提示词库','New prompt library'), label:L('提示词库名称','Prompt library name')}; render(); focusTreeEditInput('promptTreeEditInput'); return; }
    if(target.closest?.('[data-prompt-cat-new]')){
        ...
        promptTreeEdit = {kind:'category-new', value:L('新分组','New group'), label:L('分组名称','Group name')};
        pendingTreeDelete = '';
        render(); return;
    }
    if(target.closest?.('[data-prompt-cat-rename]')){
        ...
        promptTreeEdit = {kind:'category-rename', value:cat?.name || '', label:L('分组名称','Group name')};
        pendingTreeDelete = '';
        render(); return;
    }
    ...
    if(promptLibRenameBtn){
        ...
        if(libRow) activePromptLibraryId = libRow.dataset.promptLib || activePromptLibraryId;
        promptTreeFocus = 'library';
        promptTreeEdit = {kind:'library-rename', value:activePromptLibrary()?.name || '', label:L('提示词库名称','Prompt library name')};
        render(); return;
    }
...
function showDetailPreview(source, id){
    ...
    if(!['image','video'].includes(kind)){
        setStatus(L('仅图片和视频支持预览','Only images and videos support preview'));
        return;
    }
    ...
    overlay.dataset.y = '0';
    overlay.innerHTML = `
        <div class="asset-lightbox-inner" role="dialog" aria-modal="true" aria-label="${kind === 'video' ? L('视频预览','Video preview') : L('图片预览','Image preview')}">
            ${kind === 'video'
...
async function createAssetLibrary(){
    const name = window.prompt(L('资产库名称','Asset library name'), L('新资产库','New asset library'));
    if(!String(name || '').trim()) return;
...
async function saveAssetTreeEdit(){
    ...
    if(!String(name || '').trim()){
        setStatus(L('名称不能为空','Name cannot be empty'));
        return;
    }
    let data = null;
...
async function saveWorkflowTreeEdit(){
    ...
    if(workflowTreeEdit.kind === 'category-new'){
        ...
        if(!lib){
            setStatus(L('请先选择资产库','Please select an asset library first'));
            return;
        }
...
async function deleteWorkflowCategory(){
    ...
    if(pendingTreeDelete !== key){
        ...
        workflowTreeEdit = null;
        render();
        setStatus(L('再次点击确认删除工作流分组','Click again to confirm deleting workflow group'));
        return;
    }
    ...
    pendingTreeDelete = '';
    render();
    setStatus(L('工作流分组已删除','Workflow group deleted'));
}
async function renameAssetLibrary(){
    const lib = activeAssetLibrary();
    const name = window.prompt(L('资产库名称','Asset library name'), lib?.name || '');
...
async function deleteAssetLibrary(){
    ...
    if(pendingTreeDelete !== key){
        ...
        assetTreeEdit = null;
        render();
        setStatus(L('再次点击确认删除资产库','Click again to confirm deleting asset library'));
        return;
    }
...
async function createAssetCategory(){
    const name = window.prompt(L('分组名称','Group name'), L('新分组','New group'));
    if(!String(name || '').trim()) return;
...
async function renameAssetCategory(){
    const cat = activeAssetCategory();
    const name = window.prompt(L('分组名称','Group name'), cat?.name || '');
...
async function deleteAssetCategory(){
    ...
    if(pendingTreeDelete !== key){
        ...
        assetTreeEdit = null;
        render();
        setStatus(L('再次点击确认删除分组','Click again to confirm deleting group'));
        return;
    }
...
async function saveAssetEdit(id){
    ...
    if(!item || !String(name || '').trim()) {
        setStatus(L('素材名称不能为空','Asset name cannot be empty'));
        return;
    }
    const data = await apiJson(`/api/asset-library/items/${encodeURIComponent(id)}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name})});
    assetLibrary = data.library || assetLibrary;
...
async function saveAssetInlineName(id, name){
    ...
    selectedAssetId = id;
    render();
    setStatus(L('素材名称已保存','Asset name saved'));
}
async function registerAssetAvatar(id, providerId=''){
    ...
    if(!provider){ setStatus(L('请先在 API 平台管理中添加并启用 API 平台','Add and enable an API platform first')); return; }
    if(!providerAvatarSupported(provider)){ setStatus(L(`「${avatarPlatformLabel(providerAvatarPlatform(provider))}」的资产认证 API 尚未接入`,`Asset authentication API for "${avatarPlatformLabel(providerAvatarPlatform(provider))}" not integrated`)); return; }
    if(avatarBusyId) return;
    avatarBusyId = id;
    selectedAssetId = id;
    render();
    setStatus(L(`正在上传素材并提交 ${avatarPlatformLabel(providerAvatarPlatform(provider))} 审核…`,`Uploading asset and submitting to ${avatarPlatformLabel(providerAvatarPlatform(provider))} review…`));
    try {
        ...
        assetLibrary = data.library || assetLibrary;
        setStatus(L(`已提交审核，正在等待 ${avatarPlatformLabel(providerAvatarPlatform(provider))} 通过…`,`Submitted for review, waiting for ${avatarPlatformLabel(providerAvatarPlatform(provider))} approval…`));
        scheduleAvatarPoll(id, provider.id);
    } catch(err) {
        setStatus(err.message || L('数字人提交失败','Avatar submission failed'));
    } finally {
        avatarBusyId = '';
        render();
    }
...
async function checkAssetAvatarStatus(id, silent=false, providerId=''){
    ...
    if(!silent){ avatarBusyId = id; render(); setStatus(L('正在查询审核状态…','Checking review status…')); }
    try {
        ...
        if(status === 'Active') setStatus(L('审核通过，已生成 asset:// 地址，可在视频生成中通过 @ 调用','Review passed. asset:// address generated, callable via @ in video generation'));
        else if(status === 'Failed') setStatus(newReg.detail || L('审核未通过','Review failed'));
        else { setStatus(L('仍在审核中，稍后会自动刷新…','Still under review, will auto-refresh…')); scheduleAvatarPoll(id, provider.id); }
    } catch(err) {
        if(!silent) setStatus(err.message || L('查询审核状态失败','Check review status failed'));
    } finally {
        avatarBusyId = '';
        render();
    }
...
async function deleteAssetItem(id){
    ...
    pendingDeleteAssetId = '';
    render();
    setStatus(L('素材已删除','Asset deleted'));
}
async function deleteSelectedAssets(){
    ...
    pendingBatchDelete = '';
    render();
    setStatus(L(`已删除 ${data.removed || ids.length} 个素材`,`Deleted ${data.removed || ids.length} assets`));
}
function setAssetClipboard(mode){
    ...
    pendingBatchDelete = '';
    render();
    const label = mode === 'cut' ? L('剪切','Cut') : L('复制','Copy');
    setStatus(L(`${label}了 ${assetClipboard.ids.length} 个素材，切换分组后粘贴`,`${label} ${assetClipboard.ids.length} assets. Switch group then paste`));
}
async function pasteAssetClipboard(){
    ...
    if(assetClipboard.mode === 'cut'){
        ...
        assetLibrary = data.library || assetLibrary;
        setStatus(L(`已移动 ${data.moved || 0} 个素材`,`Moved ${data.moved || 0} assets`));
    } else {
        ...
        assetLibrary = data.library || assetLibrary;
        setStatus(L(`已复制 ${data.items?.length || 0} 个素材`,`Copied ${data.items?.length || 0} assets`));
    }
    assetClipboard = null;
    selectedAssetIds.clear();
    selectedAssetId = '';
...
function setLocalClipboard(mode, ids=null){
    ...
    localClipboard = {
        ...
        items,
        sourceRootName:activeSharedFolderName || L('共享文件夹','Shared folder')
    };
    selectedLocalIds.clear();
    pendingBatchDelete = '';
    render();
    setStatus(L(`复制了 ${items.length} 个共享素材，导入后会拷贝到图片资产分组（共享文件夹原文件保留）`,`Copied ${items.length} shared assets. Import copies them into an image asset group (original shared files kept)`));
}
async function pasteLocalClipboardToAssets(){
    if(!localClipboard?.items?.length) return;
    if(!activeAssetCategory()){
        setStatus(L('请先在图片资产中创建或选择分组','Create or select a group in image assets first'));
        return;
    }
    ...
    setStatus(L('正在导入共享素材...','Importing shared assets...'));
    let imported = 0;
    try {
        ...
        setStatus(err.message || L('导入共享素材失败','Import shared assets failed'));
        return;
    }
    localClipboard = null;
    selectedLocalIds.clear();
    selectedLocalId = '';
    render();
    setStatus(L(`已导入 ${imported} 个素材到图片资产`,`Imported ${imported} assets into image assets`));
}
...
async function createPromptLibrary(){
    const name = window.prompt(L('提示词库名称','Prompt library name'), L('新提示词库','New prompt library'));
    if(!String(name || '').trim()) return;
...
async function deletePromptCategory(){
    if(activePromptCategory === 'all'){
        setStatus(L('请选择具体分组后删除','Select a specific group to delete'));
        return;
    }
    ...
    pendingTreeDelete = '';
    render();
    setStatus(L('分组已删除','Group deleted'));
}
async function renamePromptLibrary(){
    const lib = activePromptLibrary();
    const name = window.prompt(L('提示词库名称','Prompt library name'), lib?.name || '');
    if(!lib || !String(name || '').trim()) return;
...
async function deletePromptLibrary(){
    const lib = activePromptLibrary();
    if(!lib) return;
    if(isSystemPromptLibrary(lib)){ setStatus(L('系统提示词库不能删除','System prompt library cannot be deleted')); return; }
    const key = `prompt-lib:${lib.id}`;
    if(pendingTreeDelete !== key){
        pendingTreeDelete = key;
        promptTreeEdit = null;
        render();
        setStatus(L('再次点击确认删除提示词库','Click again to confirm deleting prompt library'));
        return;
    }
    ...
    pendingTreeDelete = '';
    render();
    setStatus(L('提示词库已删除','Prompt library deleted'));
}
async function createPromptItem(){
    const lib = activePromptLibrary();
    if(!lib) return;
    const name = window.prompt(L('提示词名称','Prompt name'), L('新提示词','New prompt'));
    if(!String(name || '').trim()) return;
    const scene = window.prompt(L('用途说明','Usage description'), '') || '';
    const positive = window.prompt(L('正向提示词内容','Positive prompt'), '');
    if(!String(positive || '').trim()) return;
    const negative = window.prompt(L('负向提示词内容','Negative prompt'), '') || '';
...
async function savePromptCreate(){
    ...
    if(!String(name || '').trim() || !String(positive || '').trim()){
        setStatus(L('名称和正向提示词不能为空','Name and positive prompt cannot be empty'));
        return;
    }
    const category = activePromptCategory === 'all' ? 'custom' : activePromptCategory;
    const data = await apiJson('/api/prompt-libraries/items', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:lib.id, name, positive, negative, category, scene})});
    ...
    promptCreateMode = false;
    render();
    setStatus(L('提示词已新增','Prompt added'));
}
async function editPromptItem(id){
    const item = findPromptItem(id);
    const lib = activePromptLibrary();
    if(!item || !lib) return;
    const name = window.prompt(L('提示词名称','Prompt name'), item.name || '');
    if(!String(name || '').trim()) return;
    const scene = window.prompt(L('用途说明','Usage description'), item.scene || '') || '';
    const positive = window.prompt(L('正向提示词内容','Positive prompt'), item.positive || '');
    if(!String(positive || '').trim()) return;
    const negative = window.prompt(L('负向提示词内容','Negative prompt'), item.negative || '') || '';
...
async function savePromptEdit(id){
    ...
    promptEditMode = false;
    render();
    setStatus(L('提示词已保存','Prompt saved'));
}
async function deletePromptItem(id){
    ...
    pendingDeletePromptId = '';
    render();
    setStatus(L('提示词已删除','Prompt deleted'));
}
async function deleteSelectedPrompts(){
    ...
    if(pendingBatchDelete !== 'prompt'){
        ...
        render();
        setStatus(L('再次点击确认删除所选提示词','Click again to confirm deleting selected prompts'));
        return;
    }
...
root.addEventListener('click', event => {
    handleClick(event).catch(err => setStatus(err.message || L('操作失败','Operation failed')));
});
...
document.addEventListener('keydown', event => {
    ...
    if(event.target?.id === 'assetTreeEditInput'){
        if(event.key === 'Enter'){ event.preventDefault(); saveAssetTreeEdit().catch(err => setStatus(err.message || L('保存失败','Save failed'))); }
        if(event.key === 'Escape'){ event.preventDefault(); assetTreeEdit = null; render(); }
    }
    if(event.target?.id === 'workflowTreeEditInput'){
        if(event.key === 'Enter'){ event.preventDefault(); saveWorkflowTreeEdit().catch(err => setStatus(err.message || L('保存失败','Save failed'))); }
        if(event.key === 'Escape'){ event.preventDefault(); workflowTreeEdit = null; render(); }
    }
    if(event.target?.id === 'promptTreeEditInput'){
        if(event.key === 'Enter'){ event.preventDefault(); savePromptTreeEdit().catch(err => setStatus(err.message || L('保存失败','Save failed'))); }
...
root.addEventListener('change', event => {
    ...
    if(inlineLocalUploadName){
        saveLocalUploadInlineName(inlineLocalUploadName.dataset.localupInlineName || '', inlineLocalUploadName.value || '').catch(err => setStatus(err.message || L('保存失败','Save failed')));
        return;
    }
    const inlineAssetName = event.target.closest?.('[data-asset-inline-name]');
    if(inlineAssetName){
        saveAssetInlineName(inlineAssetName.dataset.assetInlineName || '', inlineAssetName.value || '').catch(err => setStatus(err.message || L('保存失败','Save failed')));
        return;
    }
    ...
    if(inlineWorkflowName){
        apiJson(`/api/asset-library/items/${encodeURIComponent(inlineWorkflowName.dataset.workflowInlineName || '')}`, {
            ...
            assetLibrary = data.library || assetLibrary;
            render();
            setStatus(L('已保存工作流名称','Workflow name saved'));
        }).catch(err => setStatus(err.message || L('保存失败','Save failed')));
        return;
    }
...
root.addEventListener('drop', event => {
    ...
    else if(drop.id === 'workflowDrop') uploadWorkflowFiles(event.dataTransfer.files).catch(err => setStatus(err.message || L('上传失败','Upload failed')));
    else uploadFiles(event.dataTransfer.files).catch(err => setStatus(err.message || L('上传失败','Upload failed')));
});
uploadInput?.addEventListener('change', event => {
    const files = event.target.files;
    if(files?.length){
        if(activeTab === 'local') uploadLocalAssets(files);
        else if(activeTab === 'workflows') uploadWorkflowFiles(files).catch(err => setStatus(err.message || L('上传失败','Upload failed')));
        else uploadFiles(files).catch(err => setStatus(err.message || L('上传失败','Upload failed')));
    }
    event.target.value = '';
});
...
refreshBtn?.addEventListener('click', () => loadAll().catch(err => setStatus(err.message || L('加载失败','Load failed'))));
storageSettingsBtn?.addEventListener('click', () => openStorageSettings().catch(err => setStatus(err.message || L('打开偏好设置失败','Open preferences failed'))));
window.addEventListener('message', event => {
    if(event.data?.type === 'studio-theme') window.StudioTheme?.apply?.(event.data.theme);
});
document.addEventListener('DOMContentLoaded', () => loadAll().catch(err => setStatus(err.message || L('加载失败','Load failed'))));
